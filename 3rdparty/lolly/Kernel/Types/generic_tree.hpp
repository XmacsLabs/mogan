...
public:
  blackbox data;
  template <typename T>
  inline generic_rep (const T& x)
      : tree_rep (type_helper<T>::id), data (close_box<T> (x)) {}
  friend class tree;
};
...
template <typename T, typename F> struct convert_helper {
  static inline T op (const F& x) { return (T) x; }
};

template <typename F> struct convert_helper<tree, F> {
  static inline tree op (const F& data) {
    return tree ((tree_rep*) tm_new<generic_rep> (data));
  }
};

template <typename T> struct convert_helper<T, tree> {
