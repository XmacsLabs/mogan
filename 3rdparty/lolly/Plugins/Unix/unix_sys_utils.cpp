...
struct _mutex_lock {
  pthread_mutex_t* rep;
  inline _mutex_lock (_mutex& m) : rep (&(m.rep)) { pthread_mutex_lock (rep); }
  inline ~_mutex_lock () { pthread_mutex_unlock (rep); }
};
...
struct _file_actions_t {
  ...
  int                        st;
  inline _file_actions_t () { st= posix_spawn_file_actions_init (&rep); }
  inline ~_file_actions_t () { posix_spawn_file_actions_destroy (&rep); }
  inline int status () const { return st; }
};
