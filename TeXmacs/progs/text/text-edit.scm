...
(tm-define (customizable-parameters t)
  ...
  (let* ((arity (tree-arity t))
         (param-count (quotient (- arity 1) 2)))
    (let loop ((i 0) (params '()))
      (if (>= i param-count)
          (reverse params)
          (let ((param-name (tree->string (tree-ref t (* 2 i)))))
            (loop (+ i 1)
                  (cond ((== param-name "color")
                         (cons (list "color" "Color") params))
                        ((== param-name "bg-color")
                         (cons (list "bg-color" "Background color") params))
                        (else params))))))))
