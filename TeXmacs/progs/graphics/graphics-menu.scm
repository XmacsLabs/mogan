...
(menu-bind graphics-mode-menu
  ...
  (assuming (style-has? "std-markup-dtd")
    ...
    (with u (list-difference gr-tags-user '(arrow-with-text arrow-with-text* circle std-arc sector))
      (with l (list-filter u (lambda (s) (style-has? (symbol->string s))))
        (assuming (nnull? l)
          ---
          (for (tag (sort l symbol<=?))
...
(tm-menu (graphics-insert-icons)
  ...
  ((check (balloon (icon "tm_arc_mode.xpm") "Insert arcs")
          "v" (== (graphics-mode) '(edit std-arc)))
   (graphics-set-mode '(edit std-arc)))
  ((check (balloon (icon "tm_arc_mode.xpm") "Insert sectors")
          "v" (== (graphics-mode) '(edit sector)))
   (graphics-set-mode '(edit sector)))
