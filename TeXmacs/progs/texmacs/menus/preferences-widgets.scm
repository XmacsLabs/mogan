  ...
  (hlist
    ...
    (vlist
      (aligned
        ...
        (assuming (qt-gui?)
          (meti (hlist // (text "Use left mode toolbar"))
            (toggle (set-boolean-preference "gui:left mode toolbar" answer)
                    (get-boolean-preference "gui:left mode toolbar"))))
        (meti (hlist // (text "Use UTF-8 for CJK in TM format"))
          (toggle (set-boolean-preference "tm format with utf8" answer)
                  (get-boolean-preference "tm format with utf8")))
        (assuming (os-macos?)
