...
### 测试修复在词内开始宏补全时补全框位置计算错误
1. 插入 `\abstract`，按 esc 关闭补全框
2. 按方向左键，移动到词内部
3. 删除一个字母，例如`s`
5. 检查：补全框弹出，且位置正确

### 测试`修复: 补全框的位置不正确`
1. `cmd--` 缩小页面直到上方出现大的灰色空隙
2. 输入 `\a` (命令随意)
3. 检查：补全框的位置正确

## 2025/08/22 fix: incorrect position of completion popup
### Problem
The completion popup is displayed at the wrong vertical position when there is a top margin in the editing area.
Realted issue #ICU6RT

### Why?
Currently, the vertical position of the completion popup is computed without considering `canvas_y`, which is usually zero. However, when the zoom factor is small or the page is very wide (for example, when using the beamer theme), a top margin appears in the editing area, making `canvas_y` non-zero. As a result, the popup is mis-placed.

### How to fix it?
Pass the `canvas_y` argument into the completion popup and include it in the calculation inside `QTMCompletionPopup::getCachedPosition`.

Note that we also apply a hack from `qt_simple_widget.cpp` to ensure the position is correct.

## 2025/08/19 修复在词内开始宏补全时补全框位置计算错误
### What?
在进行光标向前进动时，添加 tp 的末尾大于 0 的条件，以处理在词内开始补全的情况。

### Why？
目前控制光标向前进动的条件为 `i < prefix_N`，也就是直接向前进动 N 次。在词尾启动补全时，这是正确的。

但是在词中启动补全时（此情况仅有宏补全会涉及），就会导致进动过度，光标移过目标位置。

### How？
添加添加 tp 的末尾大于 0 的条件。确保向前进动不会跳出当前块
