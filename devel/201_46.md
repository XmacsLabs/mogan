...
## 2026/01/07 解决特殊情况下的双重删除
#### what



#### why
1. 用户点击登录 → 触发loginRequested() → eventLoop.quit() → 立即执行dialog->deleteLater()
2. 淡出动画仍在运行（300ms）→ 完成后调用accept() → 触发WA_DeleteOnClose自动删除
3. 结果：同一对象被删除两次 → double free → crash
