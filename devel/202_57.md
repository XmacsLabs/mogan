...
## 2025/11/17
### What
1. 修复 `link-contains-inner-link?` 文件内部的内部链接识别, 有些内部链接的是没有 vertex 的，所以无法识别头部的 # 导致这个被识别为外部链接，已在 `link-contains-inner-link?` 函数中添加处理逻辑

## 2025/11/16
### What
1. macOS平台应该是按下COMMAND的时候可以跳转
2. 提示语言采用英文，使用translate翻译，并提供字典条目
