...
### 循环回列表第一个元素时候高亮第一个元素
1.进入数学模式：$
2.任意输入一个序列，比如输入 `< tab tab tab`，一直到循环回第一个元素
原效果：第一个元素不会正常高亮，末尾元素高亮
当前效果：第一个元素正常高亮
...
## 2025/11/27 循环回列表第一个元素时候高亮第一个元素
### Why
此前循环列表有问题，第一个元素不会正常高亮

### How
1. `TeXmacs/progs/math/math-edit.scm` 增加 iteration 变量，并在 tabcycle-symbols 中重新对 tab-iter 赋值给它，方便后续判断当前 Tab 循环轮次以判断是否显示列表。
