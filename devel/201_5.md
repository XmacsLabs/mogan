...
## 如何测试
### 复现多标签页曾经存在的闪退的问题
1. 打开墨干，在窗口win1中新建两个标签页tab1和tab2
2. 通过拖动的方式，将tab2独立拖拽到新的窗口win2
3. 关闭窗口win2
4. 在窗口win1的标签页tab1中输入若干文字，看看输入文字的20秒内整个软件会不会闪退

## 2025/11/27 解决 EXC_BAD_ACCESS 崩溃
### What
窗口对象被销毁后，视图历史中仍然引用这些窗口，导致访问已释放内存

### How
  - 添加了全局 `valid_windows` 哈希集来跟踪有效窗口
  - 在 `new_window()` 中将新窗口添加到有效集
  - 在 `delete_window()` 中从有效集移除窗口
  - 在 `abstract_window()` 中添加有效性检查
  - 创建了 `is_valid_window()` 辅助函数

## 2025/11/26 解决空指针带来的段错误
