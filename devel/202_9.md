...
5. - 打开任意文档，在`文档->页面`中，点击`高级页码`后的按钮，`新页码样式层`。
   - 填写`起始页`、`结束页`、`第一页`、`页码样式`，按下`确定`后检查页码是否正确显示
   - 在没有填写`起始页`、`结束页`、`第一页`时，按下`确定`后页面应该无响应，需要用户填写完整信息，或者按下`取消`退出
   - `高级页码`与`文档->页面`中其他功能独立，不影响其他功能的及时响应效果，也不会因为设置了其他功能而误设置页码样式层
...
### How
将`高级页码`下的四个控件挪到一个新窗口中，点击`新页码样式层`按钮弹出新窗口

前三项信息默认不填写。只有信息完备后点击`确定`按钮才会应用新样式层并关闭，另外点击`取消`按钮也关闭窗口但不保存
