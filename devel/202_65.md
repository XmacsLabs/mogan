...
## 如何测试
- 清除缓存 
  - Windows下是删除`%appdata%/AppData/Roaming/MoganLab`和`%appdata%/AppData/Local/MoganLab`
  - Linux下是删除 `.cache/MoganLab`
  - macOS下是删除 `~/Library/Caches/MoganLab`

- 测试文件在 `mogan/TeXmacs/tests/tmu/202_65.tmu`，检查是否是顶层列表可以用`item-nr`修改，其他的修改不了。
...
- 创建一个子列表（先退格删去item，然后再插入有序列表），看是否从1开始排序。
