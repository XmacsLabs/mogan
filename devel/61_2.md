# 61_2 Python插件
## 如何测试
1. 打开 **.\TeXmacs\tests\tmu\61_2_python_plugin.tmu** ，根据文中的描述进行测试。
2. 打开 Help -> Plugins -> Python 检查文档的正确性

## 2026/01/29 修复python插件对sympy对象的判断问题

### What

python插件在判断传入的obj对象时，使用了type(obj).__module__，但如果是sympy的symbol，它们不存在type类型，所以__module__属性是None，导致matplotlib和sympy的判断过程报错。

### How

+ 添加isinstance，先对symbol进行判断，如果不是，再进入__module__的判断过程。
+ 为matplotlib与sympy的__module__相关判断过程添加保护措施，如果属性为None，直接返回False，避免报错影响使用。

## 2025/12/2 更改Python 插件欢迎语
...
更改mogan/TeXmacs/plugins/python/bin/python.pex 文件280行

## 2025/12/1 取消对转义字符的处理，以解决单元格内的值被单元格包裹的问题

### What

对dataframe单元格内转义字符的处理导致了单元格内的值被意外包裹，因小失大。

### How

删除 **.\TeXmacs\plugins\python\bin\python.pex** 中处理转义字符的代码。

## 2025/11/28 Python插件中文文档第一版

## 2025/11/21 解决dataframe中不能显示含有转义字符的字符串的问题

### Why

dataframe单元格中如果出现转义字符，会出现单元格为空的问题。

### How

修改 **.\TeXmacs\plugins\python\bin\python.pex**，在 **convert_dataframe_to_scheme_str()** 函数中添加逻辑进行处理。

## 2025/11/21 修正macOS平台conda的其他环境的选项
### What
上一个PR遗漏了base环境之外的情况。

## 2025/11/21 修正macOS平台conda的base环境的选项
### Why
在 Windows 上：
```
${prefix}/envs/plot/python.exe
```

在 macOS 和 Linux 上：
```
${prefix}/envs/plot/bin/python
```

## 2025/11/21 支持pillow/PIL图像的显示

### What

在 Mogan 编辑器中，当用户在内置 Python 插件中使用 PIL 库进行图像输出时，无法渲染，仅会打印数据信息。

### How

修改 **.\TeXmacs\plugins\python\bin\python.pex**，在 **flush_output()** 函数中添加判断，实现 **is_pil_object()** 与 **flush_pil_image()** 函数封装逻辑。

## 2025/11/20 在macos上添加通过homebrew安装的conda
