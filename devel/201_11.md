...
## 2025/11/28 fix: 修复 2 个变体时 tab 切换选中的问题
### What
修复 Tab 循环补全中只有 2 个变体时无法循环回到第一个元素，以及高亮逻辑依赖完整组合键绑定查询而在循环时失效的问题

### How
1. **修改 `tabcycle-symbols` 函数**：
   - 移除对完整组合 `comb` 的绑定检查（即去掉 `(if (not (kbd-find-key-binding comb)) '()` 的早期返回）
   - 改为总是基于去掉所有 " tab" 后的基础序列 `pre` 来获取符号列表
   - 这样即使 `comb` 是 `"a tab tab"` 这样的循环组合，仍然能从 `"a"` 的绑定获取完整的符号列表

2. **改进 `highlight-tabcycle-symbols` 函数**：
   - 从依赖 `kbd-find-key-binding` 查询来确定高亮，改为使用 `iteration` 变量
   - 使用模运算 `(modulo iteration len)` 计算应该高亮的索引，实现真正的循环高亮
   - 索引计算：`iteration=0` 高亮第 1 个，`iteration=1` 高亮第 2 个，`iteration=2` 循环回第 1 个，以此类推

3. **修改 `math-tabcycle-menu-needed?` 函数**：
   - 将 `(> iteration 1)` 改为 `(>= iteration 1)`
   - 确保在任何 Tab 循环过程中（即 `iteration ≥ 1`）菜单都保持可见

### 如何测试
1. 进入数学模式 `$`
2. 输入仅有 2 个变体的字符，例如 `a` (绑定 `"a"` 和 `"<alpha>"`)
3. 按第一次 Tab：应该高亮 `<alpha>`
4. 按第二次 Tab：应该循环回到 `a` 并高亮
5. 继续按 Tab：应该继续循环，不出现列表消失的情况

对于多变体的情况（如 4 个候选），应该能够循环遍历所有元素最后回到第一个。
