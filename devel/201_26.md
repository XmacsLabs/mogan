...
- 点击 标签页添加按钮 一直加加到满，然后拖出来继续点击按钮，再切换回第一个窗口点击按钮，看会不会crash
- 拖拽标签页，看是否有黑边
...
## 2025/12/08 修复Windows系统上，部分屏幕缩放比无法显示菜单的问题

## 2025/11/20 修复拖动黑边问题

## 2025/11/18 优化macOS标签栏样式
- before
![before](image/201_26_before.png)
- after
![after](image/201_26_after.png)


## 2025/10/28 修复Linux平台Emoji无法显示的问题
### Why
1. Mogan所使用的freetype是使用xmake管理的freetype（开启了png选项）
2. 引入 QWKWidgets 之后，使用了Linux系统自带的 freetype

## 2025/10/23 修复maintoolbar位置错误的问题
之前会出现在标题栏

## 2025/10/22 修复 Windows平台 新建按钮点击闪烁问题
### What?
- 点击新建标签页按钮时窗口会闪烁
- 标签栏右侧和系统按钮距离过长
...
1. 防抖机制 (Debouncing)
  ...
  if (lastCallTime.isValid() && lastCallTime.msecsTo(QTime::currentTime()) < MIN_INTERVAL_MS) {
    return;
  }
  lastCallTime = QTime::currentTime();

  - static QTime lastCallTime: 静态变量，记录上次调用时间
  - MIN_INTERVAL_MS = 500: 最小间隔时间为500毫秒
  - 防抖逻辑: 如果距离上次调用时间小于500ms，直接返回，防止重复调用

2. 修改新增文档的scheme命令

  exec_delayed (scheme_cmd ("(new-document)"));

  - scheme_cmd("(new-document)"): 创建Scheme命令字符串，调用 (new-document) 函数取代原来的 (new-buffer) 函数
      ```
  ...
  - exec_delayed: 延迟执行该命令，确保在合适的时机执行
