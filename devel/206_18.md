...
1. **字体查找和加载** (`find font`, `tt_locate_all`)
   - 优化字体缓存机制
   - 减少重复的字体存在检查
   - 预加载常用字体

2. **TeX字体加载** (`load tex font`: 21ms/6次 = 3.5ms每次)
   - 优化TFM/PK文件缓存
   - 延迟加载策略

### 🟡 中优先级

3. **字体效果计算**（如有模糊/增强效果计算）
4. **布局计算**（页面分页等）
...
1. **在真实场景中监控**
   ```bash
   set TM_DEBUG_BENCH=1
   ./mogan some-document.tm
   ```

2. **针对特定操作监控**
   ```cpp
   // 在关键操作前后添加监控
   bench_start("page_layout_calculation");
   // 页面布局计算...
   bench_cumul("page_layout_calculation");
   ```

3. **阈值过滤**
   ```cpp
   // 只显示耗时超过10ms的任务
   lolly::system::bench_print(std_bench, "", 10);
   ```
...
```cpp
// 建议：字体查询结果缓存
static hashmap<string, bool> font_existence_cache;

bool font_exists_cached(string name) {
    if (font_existence_cache.contains(name)) {
        return font_existence_cache[name];
    }
    bool exists = tt_font_exists(name);
    font_existence_cache(name) = exists;
    return exists;
}
```
...
```cpp
// 添加更多性能监控点
bench_start("font_loading_total");
// 字体加载...
bench_cumul("font_loading_total");
```

## 验证结论

1. **浮点计算性能**: ✅ 极佳（0.2μs/次），无需优化
2. **字体系统性能**: ⚠️ 需要优化（1.53ms/次字体查找）
3. **文件加载性能**: ⚠️ 需要优化（3.5ms/次TeX字体加载）
...
1. **收集真实场景数据**:
   ```bash
   set TM_DEBUG_BENCH=1
   ./mogan test-document.tm 2>&1 | grep "std-bench"
   ```

2. **识别高频调用**:
   - 监控 `find font` 调用频率和模式
   - 分析字体查找的重复性
...
**总结**：已成功建立了性能监控系统，识别出了真正的性能瓶颈。建议将优化重点从浮点计算转移到字体系统和文件加载优化上。
