...
```
2. 测试文档：TeXmacs/tests/tmu/206_18.tmu

## 2026/01/28 使字体大小支持0.5的倍数
### What
- 主要将原来`font_size`相关的代码从`int`改为`double`
- 效果举例：10.51会四舍五入为10.5；10.75会四舍五入为11

> 下面为详细的项目进度情况，因为本次修改范围比较大，因此保留
...
## 视觉回归测试
  - 在 `TeXmacs/tests/tmu/206_18.tmu`中实现
...
## 性能数据分析结果（Windows平台测试）

### 1. 浮点计算性能极佳

| 任务 | 总时间 | 调用次数 | 每次调用时间 | 性能评估 |
|------|--------|----------|--------------|----------|
| `font_script_calculation` | 36ms | 180,100次 | 0.0002ms (0.2μs) | ✅ 极佳 |
| `to_tex_font_size_conversion` | 25ms | 130,000次 | 0.00019ms (0.19μs) | ✅ 极佳 |

**结论**：字体大小浮点计算性能非常优秀，无需优化。

### 2. 实际性能瓶颈识别

| 任务 | 总时间 | 调用次数 | 每次调用时间 | 性能评估 |
|------|--------|----------|--------------|----------|
| `tt_locate_all` | 98ms | - | - | 🔴 高优先级 |
| `tt_font_exists NotoSerifCJK-Regular` | 108ms | - | - | 🔴 高优先级 |
| `find font` | 23ms | 15次 | 1.53ms | 🔴 高优先级 |
| `load tex font` | 21ms | 6次 | 3.5ms | 🔴 高优先级 |
| `decode tfm` | 0ms | 6次 | 0ms | 🟢 良好 |
| `load tt face` | 0ms | 6次 | 0ms | 🟢 良好 |

# 第十三阶段完成情况：性能优化补充

基于性能监控分析结果，已实施以下优化：

## 1. TrueType字体存在性缓存优化 (`tt_file.cpp`)

### 优化内容
- 将 `tt_fonts` 从 `hashset<string>` 改为 `hashmap<string, bool>`，存储字体存在状态
- 当字体已知不存在时快速返回 `false`，避免重复查找
...
// 修改后：存储存在状态（true=存在，false=不存在）
static hashmap<string, bool> tt_fonts;
```

### 优化效果
- 减少了重复的字体存在检查开销
- 当字体确认不存在时，后续查询直接返回 `false`，无需重复搜索

## 2. TFM/PK内存缓存 (`load_tex.cpp`)

### 优化内容
- 添加 `tfm_cache` 和 `pk_cache` 内存缓存
- 避免重复磁盘加载 TFM 和 PK 文件
- 缓存键格式：`family:size:dsize` (TFM) 和 `family:size:dpi:dsize` (PK)

### 代码实现
```cpp
// 在 load_tex.cpp 中添加内存缓存
static hashmap<string, tex_font_metric> tfm_cache;
static hashmap<string, font_glyphs> pk_cache;

// 在 load_tex_tfm() 和 load_tex_pk() 函数中添加缓存检查
...
### 性能改进
1. **字体存在性检查**：减少了重复的字体存在检查开销
2. **文件加载**：避免了 TFM/PK 文件的重复磁盘加载
3. **系统响应**：提升了字体加载的整体性能

### 性能对比表格(Windows平台测试)

基于三次测试运行的平均数据对比（单位：ms）：

| 性能指标 | 优化前 | 优化后（平均） | 变化 | 评估 |
|----------|--------|----------------|------|------|
| **浮点计算性能** | | | | |
| `font_script_calculation` (180,100次) | 36ms | 36.0ms | 0% | ✅ 保持优异 |
| `to_tex_font_size_conversion` (130,000次) | 25ms | 24.3ms | -2.8% | ✅ 保持优异 |
| **字体系统性能** | | | | |
| `tt_locate_all` | 98ms | 108.7ms | +10.9% | ⚠️ 轻微波动 |
| `tt_font_exists NotoSerifCJK-Regular` | 108ms | 113.0ms | +4.6% | ⚠️ 轻微波动 |
| `find font` (15次) | 23ms | 23.3ms | +1.3% | ⚠️ 基本持平 |
| **文件加载性能** | | | | |
| `load tex font` (6次) | 21ms | 19.3ms | -8.1% | ✅ 有所改善 |
| `decode tfm` (6次) | 0ms | 0ms | 0% | ✅ 保持最佳 |
| `load tt face` (6次) | 0ms | 2.0ms | +2ms | ⚠️ 轻微增加 |

**注**：
1. 优化前数据来自第十二阶段性能分析（见上文）
2. 优化后数据基于三次测试运行的平均值（103/111/112ms, 105/111/123ms, 12/35/23ms, 10/33/15ms, 0/0/0ms, 2/2/2ms）
3. 浮点计算性能保持优异（0.2μs/次），无需进一步优化
4. 字体系统性能受系统资源波动影响较大，优化效果需长期观察
5. 文件加载性能有所改善，`load tex font` 减少 8.1%
6. Windows平台优化不明显，macOS上明显很多
