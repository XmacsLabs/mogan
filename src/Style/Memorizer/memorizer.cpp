...
memorizer_rep::compute () {
  FAIL_WITH_MSG ("memorizer can't compute its own output");
}

void
memorizer_rep::set_children (memorizer* a, int n) {
  (void) a; (void) n;
  FAIL_WITH_MSG ("not a compound memorizer");
}
...
memorizer_rep::set_environment (environment env) {
  (void) env;
  FAIL_WITH_MSG ("not an environment memorizer");
}
...
memorizer_rep::set_tree (tree t) {
  (void) t;
  FAIL_WITH_MSG ("not a tree memorizer");
}
...
bigmem_remove (memorizer_ptr ptr) {
  ...
  FAIL_WITH_MSG ("pointer not found");
}
