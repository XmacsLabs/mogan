...
struct compound_font_rep : font_rep {
  ...
  compound_font_rep (string name, scheme_tree def, array<font> fn, double hzf,
                     double vzf);
...
compound_font_rep::compound_font_rep (string name, scheme_tree def2,
                                      array<font> fn2, double hzf2, double vzf2)
    : font_rep (name, fn2[0]), def (def2), fn (fn2),
      cm (load_charmap (map_car (def))), hzf (hzf2), vzf (vzf2) {}
...
compound_font_rep::advance (string s, int& pos, string& r, int& ch) {
  ...
  if (ch > 0 && is_nil (fn[ch])) {
    ...
    else fn[ch]= find_magnified_font (t, hzf, vzf);
