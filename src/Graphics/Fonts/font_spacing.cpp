...
get_spacing_desc (int spacing_id) {
  ASSERT (spacing_id_desc->contains (spacing_id), "invalid spacing identifier");
...
font_rep::get_spacing_entry (int mode, tree t, int i) {
  ...
  case SPC_NONE:
    return space (0);
  case SPC_THIN_SPACE:
    if (t == "default" || t == "old" || t == "wide") return (6 * spc) / 10;
    return get_spacing_entry (mode, t, i, "thin-space");
  case SPC_SPACE:
    if (t == "default" || t == "old" || t == "wide") return spc;
  ...
  case SPC_PERIOD:
    if (t == "default" || t == "old" || t == "wide") return spc + extra;
  ...
  case SPC_APPLY:
    if (t == "default" || t == "wide") {
      if (mode >= 0)
        return space (mspc->min >> 1, mspc->def >> 1, mspc->max >> 1);
...
font_rep::get_spacing_entry (int mode, tree t, int i, string kind) {
  if (is_tuple (t)) {
    for (int k= 0; k < N (t); k+= 2)
      if (k == N (t) - 1) return get_spacing_entry (mode, t[k], i);
      else if (t[k] == kind) return get_spacing_val (mode, t[k + 1]);
  }
