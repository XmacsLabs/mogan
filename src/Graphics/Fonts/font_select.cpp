...
family_features (string f) {
  ...
  if (font_features->contains (tree (f))) {
    ...
    if (is_func (t, TUPLE) && N (t) >= 1 && is_atomic (t[0])) {
      for (int i= 1; i < N (t); i++)
        if (is_atomic (t[i])) r << t[i]->label;
      for (int i= 0; i < N (r); i++)
        r[i]= normalize_feature (r[i]);
      return r;
    }
  }
  if (occurs ("Mono", f) || occurs ("Console", f) || occurs ("Typewriter", f))
    r << string ("Mono");
  if (occurs ("ArtPen", f) || occurs ("Art Pen", f)) r << string ("ArtPen");
  else if (occurs ("Pen ", f) || ends (f, "Pen")) r << string ("Pen");
  if (occurs ("Sans", f)) r << string ("SansSerif");
  if (occurs ("DemiCondensed", f) || occurs ("Demi Condensed", f))
    r << string ("DemiCondensed");
  else if (occurs ("Condensed", f) || occurs ("Narrow", f))
    r << string ("Condensed");
  else if (occurs ("Caption", f)) r << string ("Wide");
  for (int i= 0; i < N (r); i++)
    r[i]= normalize_feature (r[i]);
...
style_features (string s) {
  ...
  string r;
  for (int i= 0; i < N (s); i++)
    if (is_upcase (s[i]) && (i + 1 < N (s)) && is_locase (s[i + 1]))
      r << ' ' << s[i];
    else r << s[i];
  ...
  r= "";
  for (int i= 0; i < N (v); i++) {
    if (i + 1 < N (v) && v[i] == "Regular" &&
        (v[i + 1] == "Thin" || v[i + 1] == "Light" || v[i + 1] == "Heavy" ||
         v[i + 1] == "Black"))
      continue;
    if (N (r) == 0 || v[i - 1] == "Demi" || v[i - 1] == "Extra" ||
        v[i - 1] == "Semi" || v[i - 1] == "Ultra" || v[i - 1] == "Small")
      ;
    else r << ' ';
    if (v[i] == "Regular")
      ;
  ...
  array<string> w;
  for (int i= 0; i < N (v); i++)
    if (v[i] != "") w << normalize_feature (v[i]);
...
glyph_features (string family, string style) {
  ...
  for (int i= 0; i < N (a); i++)
    if (is_glyphs (locase_all (a[i]))) r << normalize_feature (a[i]);
  return r;
}
...
family_to_master (string f) {
  ...
  font_database_load ();
  if (!font_features->contains (tree (f)) && f != "tcx" && f != "tc") {
...
master_to_families (string m) {
  ...
  font_database_load ();
  if (!font_variants->contains (tree (m)) && m != "tcx" && m != "tc") {
...
bool
is_stretch (string s) {
  return ends (s, "condensed") || ends (s, "unextended") || ends (s, "wide") ||
         ends (s, "caption");
}

bool
is_weight (string s) {
  return ends (s, "thin") || ends (s, "light") || s == "regular" ||
         s == "medium" || ends (s, "bold") || ends (s, "heavy") ||
         ends (s, "black");
}

bool
is_slant (string s) {
  return s == "upright" || s == "italic" || s == "oblique" ||
         s == "mathitalic" || s == "mathupright" || s == "mathshape";
}

bool
is_capitalization (string s) {
  return s == "mixed" || s == "smallcaps";
}

bool
is_serif (string s) {
  return s == "serif" || s == "sansserif";
}

bool
is_spacing (string s) {
  return s == "typewriter" || s == "mono" || s == "proportional";
}

bool
is_device (string s) {
  return s == "print" || s == "typewriter" || s == "digital" || s == "pen" ||
         s == "artpen" || s == "chalk" || s == "marker";
}

bool
is_category (string s) {
  return s == "ancient" || s == "attached" || s == "calligraphic" ||
         s == "comic" || s == "decorative" || s == "distorted" ||
         s == "gothic" || s == "handwritten" || s == "initials" ||
         s == "medieval" || s == "miscellaneous" || s == "gothic" ||
         s == "outline" || s == "retro" || s == "scifi" || s == "title";
}

bool
is_glyphs (string s) {
  return s == "ascii" || s == "latin" || s == "greek" || s == "cyrillic" ||
         s == "cjk" || s == "hangul" || s == "mathsymbols" ||
         s == "mathextra" || s == "mathletters";
}

bool
is_other (string s) {
  return !is_stretch (s) && !is_weight (s) && !is_slant (s) &&
         !is_capitalization (s) && !is_serif (s) && !is_spacing (s) &&
         !is_device (s) && !is_category (s) && !is_glyphs (s) && s != "long" &&
         s != "flat";
}

bool
same_kind (string s1, string s2) {
  return (is_stretch (s1) && is_stretch (s2)) ||
...
int
distance (array<string> v, array<string> vx, array<string> w,
          array<string> wx) {
...
remove_other (array<string> a, bool keep_glyphs) {
  ...
  for (int i= 1; i < N (a); i++)
    if (!is_other (a[i]))
      if (keep_glyphs || !is_glyphs (a[i])) r << a[i];
  return r;
}
  ...
  for (int i= 0; i < N (fams); i++)
    if (N (avoid) == 0 || !contains (family_to_master (fams[i]), avoid)) {
      ...
      for (int j= 0; j < N (stys); j++) {
        ...
        if ((d1 < best_d1) || (d1 == best_d1 && d2 < best_d2) ||
...
search_font_families (array<string> v) {
  ...
  for (int i= 0; i < N (families); i++)
    if (N (search_font_styles (families[i], v)) != 0) r << families[i];
  return r;
}
...
match_properties (array<string> v, tree props) {
  for (int i= 0; i < N (props); i++)
    if (is_atomic (props[i])) {
      bool found= false;
      for (int j= 0; j < N (v); j++)
        if (v[j] == props[i]->label) found= true;
...
add_properties (array<string> v, tree props) {
  array<string> r;
  for (int i= 0; i < N (props); i++)
    if (is_atomic (props[i])) r << props[i]->label;
  r << v;
  return r;
}
