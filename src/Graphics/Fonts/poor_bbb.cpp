...
struct poor_bbb_font_rep : font_rep {
  ...
  poor_bbb_font_rep (string name, font base, double penw, double penh,
                     double fatw);
...
poor_bbb_font_rep::poor_bbb_font_rep (string name, font b, double pw, double ph,
                                      double d)
...
void
poor_bbb_font_rep::draw_fixed (renderer ren, string s, SI x, SI y, SI* xpos) {
  int i= 0;
  while (i < N (s)) {
    int start= i;
    tm_char_forwards (s, i);
