...
public:
  ...
  inline operator color () const {
    return ((int) (b * 255 + 0.5)) + (((int) (g * 255 + 0.5)) << 8) +
           (((int) (r * 255 + 0.5)) << 16) + (((int) (a * 255 + 0.5)) << 24);
  }
};
...
min (const true_color& c1, const true_color& c2) {
  return true_color (min (c1.r, c2.r), min (c1.g, c2.g), min (c1.b, c2.b),
                     min (c1.a, c2.a));
}

inline true_color
max (const true_color& c1, const true_color& c2) {
  return true_color (max (c1.r, c2.r), min (c1.g, c2.g), max (c1.b, c2.b),
                     min (c1.a, c2.a));
}
...
source_over (const true_color& c1, const true_color& c2) {
  ...
  return true_color (c1.r * f1 + c2.r * f2, c1.g * f1 + c2.g * f2,
                     c1.b * f1 + c2.b * f2, a);
}
...
towards_source (const true_color& c1, const true_color& c2) {
  ...
  return true_color (c1.r * a1 + c2.r * a2, c1.g * a1 + c2.g * a2,
                     c1.b * a1 + c2.b * a2, c1.a);
}
...
normalize (const true_color& c) {
  return true_color (max (min (c.r, 1.0), 0.0), max (min (c.g, 1.0), 0.0),
                     max (min (c.b, 1.0), 0.0), max (min (c.a, 1.0), 0.0));
}

inline true_color
hypot (const true_color& c1, const true_color& c2) {
  return true_color (
      sqrt (c1.r * c1.r + c2.r * c2.r), sqrt (c1.g * c1.g + c2.g * c2.g),
      sqrt (c1.b * c1.b + c2.b * c2.b), sqrt (c1.a * c1.a + c2.a * c2.a));
}
...
inner_max (const true_color& c1, const true_color& c2) {
  return max (max (c1.r * c2.r, c1.g * c2.g), max (c1.b * c2.b, c1.a * c2.a));
}

true_color mix (const true_color& c1, double a1, const true_color& c2,
                double a2);
true_color mix (const true_color& c1, double a1, const true_color& c2,
                double a2, const true_color& c3, double a3,
