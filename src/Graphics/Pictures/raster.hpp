...
template <typename C> class raster_rep : public abstract_struct {
public:
  int w, h;
  int ox, oy;
  C*  a;
...
template <typename C> class raster {
  ...
  inline raster (int w, int h, int ox, int oy)
      : rep (tm_new<raster_rep<C>> (w, h, ox, oy)) {
    INC_COUNT (rep);
  }
};
...
extend_border (raster<C> r, int lb, int bb, int rb, int tb) {
  return change_extents (r, r->w + lb + rb, r->h + bb + tb, r->ox + lb,
                         r->oy + bb);
}
...
template <typename Op, typename C> void foreach (raster<C>& r) {
...
template <typename Op, typename C, typename S>
raster<C>
...
template <typename C>
inline raster<C>
copy (const raster<C>& r) {
  return map<copy_op> (r);
}
template <typename C>
inline raster<C>
mul_alpha (const raster<C>& r) {
  return map<mul_alpha_op> (r);
}
template <typename C>
inline raster<C>
div_alpha (const raster<C>& r) {
  return map<div_alpha_op> (r);
}
template <typename C>
inline raster<C>
normalize (const raster<C>& r) {
  return map<normalize_op> (r);
}
template <typename C>
inline raster<typename C::scalar_type>
get_alpha (const raster<C>& r) {
  return map<get_alpha_op> (r);
}
...
template <composition_mode M, typename C, typename S>
void
...
pixelize (F fun, int w, int h, int ox, int oy, int shrink= 5) {
  ...
  for (int y= 0; y < h; y++)
    for (int x= 0; x < w; x++) {
      double cx= x - ox;
      double cy= y - oy;
      if (shrink == 1) ret->a[y * w + x]= fun.eval (cx, cy);
      else {
        C r;
        clear (r);
...
struct chi_rectangular {
  ...
  inline chi_rectangular (double rx, double ry, double phi)
      : xx (cos (phi) / rx), xy (sin (phi) / rx), yx (-sin (phi) / ry),
        yy (cos (phi) / ry) {}
...
struct chi_motion {
  ...
  inline chi_motion (double r2, double phi)
      : xx (cos (phi)), xy (sin (phi)), yx (-sin (phi)), yy (cos (phi)),
        r (r2) {}
  inline double eval (double x, double y) {
    double tx= xx * x + xy * y, ty= yx * x + yy * y;
...
rectangular_thicken (raster<C> ras, double dx, double dy, double phi) {
  raster<double> pen= rectangular_pen<double> (dx / 2 + 0.5, dy / 2 + 0.5, phi);
...
template <typename C>
raster<C>
gravitational_shadow (raster<C> s, int R, double expon, color col, double phi) {
  typedef typename C::scalar_type F;
...
raster<double> turbulence (int w, int h, int ox, int oy, long seed,
                           double wavelen_x, double wavelen_y, int nNumOctaves,
                           bool bFractalSum);

raster<double> turbulence (raster<double> ras, long seed, double wavelen_x,
                           double wavelen_y, int nNumOctaves, bool bFractalSum);

raster<true_color> turbulence (raster<true_color> ras, long seed,
...
degrade (raster<C> r, double wlx, double wly, double th, double sh) {
  ...
  raster<double> a=
      turbulence (r->w, r->h, r->ox, r->oy, 12321, wlx, wly, 3, true);
  int n= r->w * r->h;
  for (int i= 0; i < n; i++) {
    double v= a->a[i];
    if (v <= a1) a->a[i]= 1.0;
...
template <typename C>
raster<C>
translate (raster<C> r, raster<double> rdx, raster<double> rdy, double rx,
           double ry, bool tiled= false) {
  ...
  for (int y= 0; y < h; y++)
    for (int x= 0; x < w; x++) {
      ...
      if (xx >= 0 && xx + 1 < w && yy >= 0 && yy + 1 < h) {
        ...
        if (xx >= -1 && xx + 1 < w && yy >= 0 && yy < h)
          v10= r->a[yy * w + xx + 1];
        else clear (v10);
        if (xx >= 0 && xx < w && yy >= -1 && yy + 1 < h)
          v10= r->a[yy * w + xx + w];
        else clear (v01);
        if (xx >= -1 && xx + 1 < w && yy >= -1 && yy + 1 < h)
          v10= r->a[yy * w + xx + w + 1];
        else clear (v11);
      }
...
distort (raster<C> r, double wlx, double wly, double rx, double ry) {
  ...
  raster<double> rdx=
      turbulence (r->w, r->h, r->ox, r->oy, 12345, wlx, wly, 3, true);
  raster<double> rdy=
      turbulence (r->w, r->h, r->ox, r->oy, 54321, wlx, wly, 3, true);
...
tiled_distort (raster<C> r, double rx, double ry) {
  raster<double> rdx=
      turbulence (r->w, r->h, r->ox, r->oy, -12345, r->w, r->h, 3, true);
  raster<double> rdy=
      turbulence (r->w, r->h, r->ox, r->oy, -54321, r->w, r->h, 3, true);
