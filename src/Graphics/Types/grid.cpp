...
cartesian_rep::get_curves (point lim1, point lim2, bool b) {
  ...
  if (!b && yo >= y1 && yo <= y2) res << create_line (x1, yo, x2, yo, col[0]);
  if (!b && xo >= x1 && xo <= x2) res << create_line (xo, y1, xo, y2, col[0]);
  return res;
}
...
cartesian_rep::find_closest_point (point p, point pmin, point pmax) {
  ...
  for (i= 1; i < N (subd); i++) {
    ...
    oldx= res[0];
    oldy= res[1];
    if (inside_rectangle (res, pmin, pmax)) return res;
  }
  return res;
}
...
static grid_curve
create_arc (double x1, double y1, double x2, double y2, double x3, double y3,
            string col) {
  array<point> a (3);
  a[0]= point (x1, y1);
  a[1]= point (x2, y2);
...
polar_rep::get_curves (point lim1, point lim2, bool b) {
  ...
  if (x1 <= 0 && y1 <= 0 && x2 >= 0 && y2 >= 0) {
    ...
    double ox= (x1 + x2) / 2;
    double oy= (y1 + y2) / 2;
    if (oy >= 0) P1= point (0, y1 >= 0 ? y1 : 0);
    else P1= point (0, y2 <= 0 ? y2 : 0);

    if (ox >= 0 && oy >= 0) P2= point (x2, y2);
    else if (ox <= 0 && oy >= 0) P2= point (x1, y2);
    else if (ox <= 0 && oy <= 0) P2= point (x1, y1);
    else if (ox >= 0 && oy <= 0) P2= point (x2, y1);
  }
  ...
  for (i= N (subd) - 1; i >= 1; i--) {
    ...
    if (nsub != 0) {
      ...
      for (j= 0; j < astep * nsub; j++)
        res << create_line (
            xo, yo, xo + R2 * cos ((2 * tm_PI * j) / (astep * nsub)),
            yo + R2 * sin ((2 * tm_PI * j) / (astep * nsub)), col[i]);
    }
  }
...
polar_rep::find_closest_point (point p, point pmin, point pmax) {
  ...
  for (i= 1; i < N (subd); i++) {
    ...
    if (fnull (norm (p), 1.0e-6)) a= 0.0;
    else a= nearest ((arg (p) / (2 * tm_PI)) * astep * ssubd);
    n= n * (step / ssubd);
    a= a / (astep * ssubd);
    if (i != 1) {
...
as_grid (tree t) {
  ...
  if (is_tuple (t, "empty")) gr= empty_grid ();
  else if (is_tuple (t, "cartesian")) {
    if (is_tuple (t, "cartesian", 0))
      ;
    else if (is_tuple (t, "cartesian", 1)) step= as_double (t[1]);
    else if (is_tuple (t, "cartesian", 2)) {
      center= as_point (t[1]);
      step  = as_double (t[2]);
    }
    gr= cartesian (subd, col, center, step);
  }
  else if (is_tuple (t, "polar")) {
    SI astep= 8;
    if (is_tuple (t, "polar", 0))
      ;
    else if (is_tuple (t, "polar", 1)) step= as_double (t[1]);
    else if (is_tuple (t, "polar", 2)) {
      step = as_double (t[1]);
      astep= as_int (t[2]);
    }
    else if (is_tuple (t, "polar", 3)) {
      center= as_point (t[1]);
      step  = as_double (t[2]);
      astep = as_int (t[3]);
    }
  ...
  else if (is_tuple (t, "logarithmic")) {
    SI base= 10;
    if (is_tuple (t, "logarithmic", 0))
      ;
    else if (is_tuple (t, "logarithmic", 1)) step= as_double (t[1]);
    else if (is_tuple (t, "logarithmic", 2)) {
      step= as_double (t[1]);
      base= as_int (t[2]);
    }
    else if (is_tuple (t, "logarithmic", 3)) {
      center= as_point (t[1]);
      step  = as_double (t[2]);
      base  = as_int (t[3]);
    }
