...
can_simplify (array<point> a, int i1, int i2, double eps) {
  for (int i= i1 + 1; i < i2; i++)
    if (seg_dist (a[i1], a[i2], a[i]) > eps) return false;
  return true;
}
...
simplify_polyline (array<point> a, double eps) {
  ...
  while (last < n - 1) {
    int step= 1;
    while (last + (2 * step) < n &&
           can_simplify (a, last, last + 2 * step, eps))
      step= step << 1;
    int next= last + step;
    while (step != 1) {
      step= step >> 1;
...
eval_bezier (array<point> bez, double t) {
  ...
  double u2= u * u;
  double u3= u * u2;
  return u3 * bez[0] + (3.0 * u2 * t) * bez[1] + (3.0 * u * t2) * bez[2] +
         t3 * bez[3];
}
...
rectify_bezier (array<point>& r, array<point> bez, double t0, double t1,
  ...
  if (d1 < eps && d2 < eps && d3 < eps) r << eval_bezier (bez, t1);
  else {
