...
search (glyph gl, int j, int pw, int serial, int& i, int& rj) {
  ...
  while (serial > 0 && i < ww) {
    if (start)
      while (i < ww && gl->get_x (i, j) == 0)
        i++;
    else
      while (i < ww && !is_empty (gl, i, i + pw - 1, j))
        i++;
    serial--;
    start= !start;
  }
...
unserif (glyph gl, int kind, SI penw, int serial) {
  ...
  if (kind == UNSERIF_BOTTOM) yext= (gl->yoff - hh + 1) * PIXEL;
...
unserif_one (glyph gl, int kind, SI penw) {
  ...
  case UNSERIF_TOP_LEFT:
    return unserif (unserif (gl, UNSERIF_TOP, penw, 1), UNSERIF_TOP, penw, 2);
  case UNSERIF_TOP_RIGHT:
    return unserif (unserif (gl, UNSERIF_TOP, penw, -1), UNSERIF_TOP, penw, -2);
  case UNSERIF_TOP_CENTER:
    return unserif (unserif (gl, UNSERIF_TOP, penw, 3), UNSERIF_TOP, penw, -3);
  case UNSERIF_TOP_SINGLE:
    return unserif (unserif (gl, UNSERIF_TOP, penw, 1), UNSERIF_TOP, penw, -1);
  case UNSERIF_BOTTOM_LEFT:
    return unserif (unserif (gl, UNSERIF_BOTTOM, penw, 1), UNSERIF_BOTTOM, penw,
                    2);
  case UNSERIF_BOTTOM_RIGHT:
    return unserif (unserif (gl, UNSERIF_BOTTOM, penw, -1), UNSERIF_BOTTOM,
                    penw, -2);
  case UNSERIF_BOTTOM_CENTER:
    return unserif (unserif (gl, UNSERIF_BOTTOM, penw, 3), UNSERIF_BOTTOM, penw,
                    -3);
...
unserif (glyph gl, int code, SI penw) {
  ...
  case 0x4d:
    return unserif (
        unserif (unserif (gl, 0x41, penw), UNSERIF_HALF_TOP_LEFT, penw),
  ...
  case 0x55:
  case 0x56:
    return unserif (unserif (gl, UNSERIF_TOP_LEFT, penw), UNSERIF_TOP_RIGHT,
                    penw);
  case 0x57:
