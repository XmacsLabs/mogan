...
struct stretched_font_metric_rep : public font_metric_rep {
  ...
  double                xf, yf;
  hashmap<int, pointer> ms;
  stretched_font_metric_rep (string name, font_metric fnm2, double xf2,
                             double yf2)
      : font_metric_rep (name), fnm (fnm2), xf (xf2), yf (yf2),
        ms (error_metric) {}
  bool    exists (int c) { return fnm->exists (c); }
  metric& get (int c) {
    metric& m (fnm->get (c));
      ...
      for (i= i1; i < i2; i++)
        for (j= j1; j < j2; j++)
          if (gl->get_x (i, j)) {
            ...
            Y1b       = max (Y + 0.0, Y1b);
            Y2b       = min (Y + 1.0, Y2b);
            if (X1b < X2b && Y1b < Y2b) sum+= (X2b - X1b) * (Y2b - Y1b);
          }
...
struct stretched_font_glyphs_rep : public font_glyphs_rep {
  ...
  double              xf, yf;
  hashmap<int, glyph> gs;
  stretched_font_glyphs_rep (string name, font_glyphs fng2, double xf2,
                             double yf2)
      : font_glyphs_rep (name), fng (fng2), xf (xf2), yf (yf2),
        gs (error_glyph) {}
  glyph& get (int c) {
    glyph& orig (fng->get (c));
    if ((&orig != &error_glyph) && !gs->contains (c))
...
struct extended_font_glyphs_rep : public font_glyphs_rep {
  ...
  extended_font_glyphs_rep (string name, font_glyphs fng2, double xf2, SI p2)
      : font_glyphs_rep (name), fng (fng2), xf (xf2), penw (p2),
        gs (error_glyph) {}
  glyph& get (int c) {
    glyph& orig (fng->get (c));
    if ((&orig != &error_glyph) && !gs->contains (c))
...
struct mono_font_metric_rep : public font_metric_rep {
  ...
  mono_font_metric_rep (string name, font_metric fnm2, SI lw2, SI phw2)
      : font_metric_rep (name), fnm (fnm2), lw (lw2), phw (phw2),
        ms (error_metric) {}
  bool    exists (int c) { return fnm->exists (c); }
  metric& get (int c) {
    metric& m (fnm->get (c));
...
struct mono_font_glyphs_rep : public font_glyphs_rep {
  ...
  mono_font_glyphs_rep (string name, font_glyphs fng2, SI lw2, SI phw2)
      : font_glyphs_rep (name), fng (fng2), lw (lw2), phw (phw2),
        gs (error_glyph) {}
  glyph& get (int c) {
    glyph& orig (fng->get (c));
    if ((&orig != &error_glyph) && !gs->contains (c))
...
struct bolden_font_metric_rep : public font_metric_rep {
  ...
  bolden_font_metric_rep (string name, font_metric fnm2, SI dtot2, SI dver2)
      : font_metric_rep (name), fnm (fnm2), dtot (dtot2), dver (dver2),
        ms (error_metric) {}
  bool    exists (int c) { return fnm->exists (c); }
  metric& get (int c) {
    metric& m (fnm->get (c));
...
struct bolden_font_glyphs_rep : public font_glyphs_rep {
  ...
  bolden_font_glyphs_rep (string name, font_glyphs fng2, SI dpen2, SI dtot2,
                          SI dver2)
      : font_glyphs_rep (name), fng (fng2), dpen (dpen2), dtot (dtot2),
        dver (dver2), gs (error_glyph) {}
  glyph& get (int c) {
    glyph& orig (fng->get (c));
    if ((&orig != &error_glyph) && !gs->contains (c))
...
void
adjust_bbb_offsets (glyph gl, SI delta, int next, int prev, array<int>& start,
                    array<int>& end) {
  int ww= gl->width;
  if (start[prev] == -1) {
  ...
  if (end[next] + delta < start[prev]) {
    ...
    for (int i= lo; i < hi; i++)
      if (gl->get_x (i, next) != 0) {
        int i1= i;
        while (i1 > 0 && gl->get_x (i1 - 1, next) != 0)
          i1--;
        int i2= i;
        while (i2 + 1 < ww && gl->get_x (i2 + 1, next) != 0)
          i2--;
        start[next]= i1;
        end[next]  = i2;
        return;
  ...
  if (4 * lw < 3 * penw) {
    if (true) {
      ...
      for (int j= 0; j < N (start); j++)
        if (start[j] >= 0) start[j]+= (penw - lw) / PIXEL;
      fat-= (penw - lw);
      penw= lw;
    }
  }
...
bbb_initialize () {
  ...
  bbb_right << ((int) '1') << ((int) '2') << ((int) '3') << ((int) '5')
            << ((int) '7') << ((int) '9')
            << ((int) 'J')
            //      << ((int) 'A') << ((int) 'J') << ((int) 'M')
...
struct make_bbb_font_glyphs_rep : public font_glyphs_rep {
  ...
  SI                  penw, penh, fatw;
  hashmap<int, glyph> gs;
  make_bbb_font_glyphs_rep (string name, font_glyphs fng2, SI pw, SI ph, SI fw)
      : font_glyphs_rep (name), fng (fng2), penw (pw), penh (ph), fatw (fw),
        gs (error_glyph) {}
  glyph& get (int c) {
    glyph& orig (fng->get (c));
    if ((&orig != &error_glyph) && !gs->contains (c))
...
curly (glyph gl) {
  ...
  glyph bmr (ww + 2 * padx, hh + 2 * pady, gl->xoff + padx, gl->yoff + pady,
             gl->depth);
