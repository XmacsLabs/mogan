...
operator << (tm_ostream& out, modification mod) {
  ...
  case MOD_SET_CURSOR:
    return out << "set_cursor (" << root (mod)
	       << ", " << index (mod) << ", " << mod->t << ")";
  default: FAIL_WITH_MSG ("invalid modification type");
    return out;
  }
}
...
clean_assign (tree t, path p, tree u) {
  if (is_nil (p)) return copy (u);
    ...
    if (j >= n) FAIL_WITH_MSG("clean_remove(): Invalid path."); //return copy(u);  // FIXME? check whether this is the right return value.
    tree r (t, n);
...
clean_remove (tree t, path p, int nr) {
  if (is_nil (p->next) && is_atomic (t)) {
    string s= t->label;
    if (N(s) < p->item+nr)
      FAIL_WITH_MSG ("clean_remove: Invalid remove from atomic tree");
    ...
    int i, j= p->item, n= N(t);
    if (j >= n) FAIL_WITH_MSG ("clean_remove: Invalid path"); //return t;  // FIXME? check whether this is the right return value.
    tree r (t, n);
    for (i=0; i<j; i++) r[i]= t[i];
...
clean_apply (tree t, modification mod) {
  ...
  default:
    FAIL_WITH_MSG ("invalid modification type");
    return "";
  }
}
