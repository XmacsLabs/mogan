...
public:
  ...
  static qt_widget create (types _type, X1 x1, X2 x2, X3 x3, X4 x4, X5 x5) {
    ...
    return tm_new<qt_ui_element_rep> (_type,
                                      close_box<T> (T (x1, x2, x3, x4, x5)));
  }
...
/*! Ad-hoc command to be used with choice widgets.
 ...
 \sa qt_ui_element, , qt_ui_element_rep::as_qwidget,
 qt_ui_element_rep::choice_widget
 */
...
public:
  ...
  virtual void apply () {
    if (qwid) {
      ...
      else if (selected.size () > 0) l= from_qstring (selected[0]);
      else l= "";
