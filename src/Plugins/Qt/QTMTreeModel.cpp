...
QTMTreeModel::index_from_item (const tree& tref) const {
  ...
  if (ipath_has_parent (ip)) {
    ...
    if (is_nil (p))
      return createIndex (ip->item, 0, inside (t)); // parent is root?
...
QTMTreeModel::data (const QModelIndex& index, int role) const {
  ...
  case Qt::StatusTipRole:
    ...
    else return to_qstring (t[pos]->label);
  case Qt::DecorationRole: {
    ...
    else u= resolve (url (t[pos]->label));
    if (is_rooted_name (u)) return QPixmap (to_qstring (concretize (u)));
...
qt_tree_observer_rep::~qt_tree_observer_rep () { delete model; }
...
qt_tree_observer_rep::announce (tree& ref, modification mod) {
  ...
  /* MOD_SET_CURSOR is announced only for TeXmacs buffers. We might want to also
   ...
   In this case we could tell the views that they might want to update their
   selections, e.g.
