...
#ifdef Q_OS_MAC
#define QTM_MINI_FONTSIZE                                                      \
  12 // also see uses of preference "gui:mini-fontsize"
...
public:
  ...
  SubControl hitTestComplexControl (ComplexControl             control,
                                    ...
                                    const QPoint&              pos,
                                    const QWidget*             widget= 0) const;
  QRect      itemPixmapRect (const QRect& rect, int alignment,
                             const QPixmap& pixmap) const;
  QRect      itemTextRect (const QFontMetrics& metrics, const QRect& rect,
                           int alignment, bool enabled, const QString& text) const;
