...
qt_window_widget_rep::qt_window_widget_rep (QWidget* _wid, string name,
    ...
    : qt_widget_rep (window_widget, _wid), orig_name (name), quit (_quit),
      fake (_fake) {
  qwid->setProperty ("texmacs_window_widget",
                     QVariant::fromValue ((void*) this));

  // Try to connect only if the QWidget has a closed() signal
  // We need this for the QDockWidgets we use in side tools (see
  // qt_tm_widget_rep)
  if (qwid->metaObject ()->indexOfSignal (
          QMetaObject::normalizedSignature ("closed()").constData ()) != -1) {
    QTMCommand* qtmcmd= new QTMCommand (qwid, quit);
    QObject::connect (qwid, SIGNAL (closed ()), qtmcmd, SLOT (apply ()));
  }

  if (!has_resizable_children (_wid)) qwid->setFixedSize (qwid->sizeHint ());

  // HACK: don't increment window count for side tools or any other fake windows
  if (!fake) win_id= ++nr_windows;

  if (DEBUG_QT) debug_qt << "Creating qt_window_widget " << id << "\n";
...
qt_window_widget_rep::query (slot s, int type_id) {
  ...
  case SLOT_POSITION: {
    ...
    QRect g;
    // FIXME: dock widgets are embedded into qt_window_widget_reps as a
    // temporary hack because of this the underlying widget is not always a top
    // level window
