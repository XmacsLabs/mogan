...
ScreenshotTool::ScreenshotTool (QWidget* parent)
    : QWidget (parent), isSelecting (false), isMoving (false),
      isResizing (false), currentRegion (None), devicePixelRatio (1.0) {
...
void
ScreenshotTool::paintEvent (QPaintEvent* event) {
  Q_UNUSED (event);
  ...
  if (isSelecting || !selectedRect.isNull ()) {
    ...
    if (drawRect.isValid ()) {
      ...
      // 绘制选中区域的原始内容
      painter.drawPixmap (drawRect.topLeft (),
                          screenPixmap.copy (physicalRect));
      ...
      if (drawRect.width () > 40 && drawRect.height () > 40) {
        ...
        // 四个角
        painter.drawRect (drawRect.topLeft ().x () - handleSize / 2,
                          drawRect.topLeft ().y () - handleSize / 2, handleSize,
                          handleSize);
        painter.drawRect (drawRect.topRight ().x () - handleSize / 2,
                          drawRect.topRight ().y () - handleSize / 2,
                          handleSize, handleSize);
        painter.drawRect (drawRect.bottomLeft ().x () - handleSize / 2,
                          drawRect.bottomLeft ().y () - handleSize / 2,
                          handleSize, handleSize);
        painter.drawRect (drawRect.bottomRight ().x () - handleSize / 2,
                          drawRect.bottomRight ().y () - handleSize / 2,
                          handleSize, handleSize);

        // 四条边中点
        painter.drawRect (
            drawRect.left () + drawRect.width () / 2 - handleSize / 2,
            drawRect.top () - handleSize / 2, handleSize, handleSize);
        painter.drawRect (
            drawRect.left () + drawRect.width () / 2 - handleSize / 2,
            drawRect.bottom () - handleSize / 2, handleSize, handleSize);
        painter.drawRect (drawRect.left () - handleSize / 2,
                          drawRect.top () + drawRect.height () / 2 -
                              handleSize / 2,
                          handleSize, handleSize);
        painter.drawRect (drawRect.right () - handleSize / 2,
                          drawRect.top () + drawRect.height () / 2 -
                              handleSize / 2,
                          handleSize, handleSize);
      ...
      // 绘制尺寸信息
      QString sizeText=
          QString ("%1 x %2").arg (drawRect.width ()).arg (drawRect.height ());
      ...
      // 在矩形左上角外部绘制尺寸信息背景
      QRect textRect (drawRect.topLeft ().x (), drawRect.topLeft ().y () - 25,
                      100, 20);
...
int
ScreenshotTool::getMouseRegion (const QPoint& point) const {
  ...
  QRect bottomLeftCorner (selectedRect.bottomLeft () - QPoint (margin, margin),
                          QSize (2 * margin, 2 * margin));
  QRect bottomRightCorner (selectedRect.bottomRight () -
...
void
ScreenshotTool::updateCursorShape (int region) {
  switch (region) {
  case Inside:
...
void
ScreenshotTool::adjustRect (QRect& rect, int region,
  ...
  if (rect.height () < 10) {
    ...
    else if (region == Bottom || region == BottomLeft ||
             region == BottomRight) {
...
void
ScreenshotTool::mousePressEvent (QMouseEvent* event) {
...
void
ScreenshotTool::mouseMoveEvent (QMouseEvent* event) {
...
void
ScreenshotTool::mouseReleaseEvent (QMouseEvent* event) {
  if (event->button () == Qt::LeftButton) {
    if (isSelecting) {
      endPoint    = event->pos ();
...
void
ScreenshotTool::mouseDoubleClickEvent (QMouseEvent* event) {
  if (event->button () == Qt::LeftButton && selectedRect.isValid ()) {
    captureSelectedArea ();
  }
}

void
ScreenshotTool::keyPressEvent (QKeyEvent* event) {
...
void
ScreenshotTool::captureSelectedArea () {
  if (selectedRect.isValid ()) {
    // 确保矩形在屏幕范围内
...
void
ScreenshotTool::saveScreenshot (const QPixmap& pixmap) {
  // 直接将图片复制到剪贴板
  ...
  // 创建标签
  QLabel* label= new QLabel ("✓ 截图已复制到剪贴板", toast);
  ...
  // 将提示框移动到屏幕顶部中央
  toast->move (QGuiApplication::primaryScreen ()->geometry ().center ().x () -
                   toast->width () / 2,
               50);

  // 3秒后自动关闭
  QTimer::singleShot (3000, toast, &QWidget::close);
}

void
ScreenshotTool::exitCapture () {
  hide ();
  emit aboutToClose ();
}