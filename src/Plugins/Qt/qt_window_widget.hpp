...
/*! Models the simplest top-level window possible
 ...
 Later, the handling of some texmacs messages (SLOT_WINDOW in
 qt_simple_widget_rep for instance) will require access to an instance of
 qt_window_widget which they retrieve using the static member
 widget_from_qwidget(), who in turn uses the mentioned property.
...
public:
  qt_window_widget_rep (QWidget* _wid, string name, command q,
                        bool fake= false);
