...
socket_link::socket_link (int s, SOCKADDR_STORAGE* addr) {
  ...
  QObject::connect (qsnr, SIGNAL (activated (int)), this,
                    SLOT (data_set_ready (int)));
  QObject::connect (qsnw, SIGNAL (activated (int)), this,
                    SLOT (ready_to_send (int)));
...
socket_link::~socket_link () {
  ...
  if (qsnr) {
    qsnr->disconnect (SIGNAL (activated (int)));
    tm_delete (qsnr);
  }
  if (qsnw) {
    qsnw->disconnect (SIGNAL (activated (int)));
    tm_delete (qsnw);
  }
  if (sock != -1) {
    CLOSE (sock);
...
socket_link::start () {
  ...
  case ST_OK:
    ...
    break;
  case ST_VOID:
    ret= "Socket not initialised";
    break;
  case ST_SOCKET:
    ret= "Error in opening socket";
    break;
  case ST_FCNTL:
    ret= "Error in setting blocking mode";
    break;
  case ST_BIND:
    ret= "Error during bind";
    break;
  case ST_LISTEN:
    ret= "Error during listen";
    break;
  case ST_CONNECTION:
    ret= "Error during connect";
    break;
  case ST_GETHOST:
    ret= "Error in getting host";
    break;
  case ST_NOTIF:
    ret= "Error in setting notifier";
    break;
  default:
...
socket_link::data_set_ready (int s) {
  ...
  if (lgdata == 0) {
    ...
    case ERRSOC (ECONNABORTED):
      DBG_IO ("Client disconnected");
      break;
    default:
      DBG_IO ("Receiving error: " << ERRNO);
    }
    stop ();
  }
  else {
    ...
    if (DEBUG_IO) {
      ...
      for (int i= 0; i < N (s); i++)
        if (((int) (unsigned char) s[i]) >= 128 ||
            (((int) (unsigned char) s[i]) < 32 && s[i] != '\n' && s[i] != '\t'))
          ok= false;
      if (ok) {
        DBG_IO ("Data received: " << s);
      }
      else {
        DBG_IO ("Binary data received size= " << N (s));
      }
    }
...
socket_server::socket_server (string host, unsigned short port) {
  ...
  for (rp= result; rp != NULL; rp= rp->ai_next) {
    ...
    if (sock < 0) continue;
#if !defined(OS_MINGW) && !defined(OS_WIN)
    if (fcntl (sock, F_SETFL, O_NONBLOCK) == -1) continue;
#else
    {
      using namespace wsoc;
      u_long flags= -1;
      if (ioctlsocket (sock, FIONBIO, &flags) == SOCKET_ERROR) continue;
    }
#endif
  ...
  QObject::connect (qsnc, SIGNAL (activated (int)), this,
                    SLOT (connection (int)));
  ...
  if (sclt > 0) {
    ...
    if (clt) {
      if (clt->alive ()) {
        ...
        DBG_IO ("Client Connected from " << string_from_socket_address (&cltadd)
                                         << ", with id: " << clt->getid ());
      }
      else tm_delete (clt);
    }
