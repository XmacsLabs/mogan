 ...
 * printer.
 *
 * This class provides one signal, doneReading(), which will be emmitted after
 * the system command used to read the configuration variables finishes. Why
 * bother? Because lpoptions uses cupsGetDests() from the CUPS API, which in
 * turn opens a connection to the server (using cupsGetDests2(), see cups/dest.c
 * in CUPS source code, v1.4.5). If it takes too long to reply, we are going to
...
/*!
 * A WinQTMPrinterSettings object sets the the default print command to be
 either
