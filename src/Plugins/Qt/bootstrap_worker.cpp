...
void BootstrapWorker::requestStop()
{
...
bool BootstrapWorker::performInitialization()
{
...
bool BootstrapWorker::initializeFileSystem()
{
...
bool BootstrapWorker::initializeConfiguration()
{
...
bool BootstrapWorker::initializePlugins()
{
    // This function performs time-consuming plugin initialization tasks
    // Note: Core initialization (install_status setup) should be done in main thread
    // This function focuses on background tasks that can run concurrently
    ...
    for (int i = 1; i <= totalSteps && !m_stopRequested; ++i) {
        ...
        QString message = QString("Loading plugin module %1/%2").arg(i).arg(totalSteps);

        // Calculate progress within this step
        int stepProgress = (i * 100) / totalSteps;
        int overallProgress = 60 + (stepProgress * 20 / 100); // 60-80% range for plugin init

        emit progressUpdated(static_cast<int>(Step_PluginInitialization),
...
bool BootstrapWorker::initializeSchemeEnvironment()
{
...
void BootstrapWorker::updateProgress(InitializationStep step, const QString& message)
{
    m_currentStep = step;

    // Calculate progress percentage based on step
    // Simple linear progression for now, will be enhanced with dynamic calculation
    int baseProgress = 0;
    switch (step) {
    ...
    qDebug() << "BootstrapWorker: Progress update - Step" << step << ":" << message << "(" << m_progress << "%)";
}

void BootstrapWorker::calculateDynamicProgress()
{
    ...
    if (stepEstimated > 0) {
        ...
        int intraStepProgress = qMin(100, static_cast<int>((stepElapsed * 100) / stepEstimated));

        // Map to overall progress
        int stepIndex = static_cast<int>(m_currentStep) - 1; // Convert to 0-based index
        ...
        if (totalSteps > 0) {
            ...
            m_progress = (stepsCompleted * progressPerStep) + (intraStepProgress * progressPerStep / 100);
            m_progress = qMin(100, m_progress);
        }
    }
}

void BootstrapWorker::updateTimeEstimation()
{
    qint64 currentTime = QDateTime::currentMSecsSinceEpoch();
...
qint64 BootstrapWorker::estimateStepTime(InitializationStep step) const
{
