...
QTMProxyStyle::layoutSpacing (QSizePolicy::ControlType control1,
  ...
  return baseStyle ()->layoutSpacing (control1, control2, orientation, option,
                                      widget);
}
#endif

void
QTMProxyStyle::drawComplexControl (ComplexControl             control,
...
void
QTMProxyStyle::drawControl (ControlElement element, const QStyleOption* option,
                            QPainter* painter, const QWidget* widget) const {
  baseStyle ()->drawControl (element, option, painter, widget);
}

void
QTMProxyStyle::drawItemPixmap (QPainter* painter, const QRect& rect,
                               int alignment, const QPixmap& pixmap) const {
  baseStyle ()->drawItemPixmap (painter, rect, alignment, pixmap);
}

void
QTMProxyStyle::drawItemText (QPainter* painter, const QRect& rect,
                             int alignment, const QPalette& pal, bool enabled,
                             const QString&      text,
                             QPalette::ColorRole textRole) const {
  baseStyle ()->drawItemText (painter, rect, alignment, pal, enabled, text,
                              textRole);
}

void
QTMProxyStyle::drawPrimitive (PrimitiveElement elem, const QStyleOption* option,
                              QPainter* painter, const QWidget* widget) const {
  baseStyle ()->drawPrimitive (elem, option, painter, widget);
}

QPixmap
QTMProxyStyle::generatedIconPixmap (QIcon::Mode iconMode, const QPixmap& pixmap,
                                    const QStyleOption* option) const {
  return baseStyle ()->generatedIconPixmap (iconMode, pixmap, option);
}

QStyle::SubControl
QTMProxyStyle::hitTestComplexControl (ComplexControl             control,
                                      const QStyleOptionComplex* option,
                                      const QPoint&              pos,
                                      const QWidget*             widget) const {
...
QRect
QTMProxyStyle::itemPixmapRect (const QRect& rect, int alignment,
                               const QPixmap& pixmap) const {
  return baseStyle ()->itemPixmapRect (rect, alignment, pixmap);
}

QRect
QTMProxyStyle::itemTextRect (const QFontMetrics& metrics, const QRect& rect,
...
int
QTMProxyStyle::pixelMetric (PixelMetric metric, const QStyleOption* option,
                            const QWidget* widget) const {
...
QSize
QTMProxyStyle::sizeFromContents (ContentsType type, const QStyleOption* option,
...
QIcon
QTMProxyStyle::standardIcon (StandardPixmap      standardIcon,
                             const QStyleOption* option,
                             const QWidget*      widget) const {
...
QPixmap
QTMProxyStyle::standardPixmap (StandardPixmap      standardPixmap,
                               const QStyleOption* option,
                               const QWidget*      widget) const {
...
int
QTMProxyStyle::styleHint (StyleHint hint, const QStyleOption* option,
...
QRect
QTMProxyStyle::subControlRect (ComplexControl             control,
                               const QStyleOptionComplex* option,
...
QRect
QTMProxyStyle::subElementRect (SubElement element, const QStyleOption* option,
                               const QWidget* widget) const {
...
static void
qtmDrawRoundedRect (QPainter* p, const QRectF& rect, qreal xRadius,
                    qreal yRadius, Qt::SizeMode mode) {
...
static void
qtmDrawShadeRoundPanel (QPainter* p, const QRect& r, const QPalette& pal,
  ...
  if (fill) {
    if (fill->color () == shade) shade= pal.shadow ().color ();
    if (fill->color () == light) light= pal.midlight ().color ();
  }
...
void
QTMStyle::drawPrimitive (PrimitiveElement element, const QStyleOption* opt,
  ...
  case PE_PanelButtonTool:
    if ((opt->state & (State_Sunken | State_On))) {
      qtmDrawShadeRoundPanel (
          p, opt->rect,
          QPalette (opt->palette.color (QPalette::Mid)), // opt->palette,
      ...
      qtmDrawShadeRoundPanel (
          p, opt->rect,
          opt->palette, // QPalette(opt->palette.color(QPalette::Mid)),//opt->palette,
...
void
QTMStyle::drawComplexControl (ComplexControl cc, const QStyleOptionComplex* opt,
                              QPainter* p, const QWidget* widget) const {
  switch (cc) {
  case CC_ToolButton:
    if (const QStyleOptionToolButton* toolbutton=
      ...
      menuarea=
          proxy ()->subControlRect (cc, toolbutton, SC_ToolButtonMenu, widget);
      ...
      if (toolbutton->subControls & SC_ToolButtonMenu) {
        ...
        int mbi=
            proxy ()->pixelMetric (PM_MenuButtonIndicator, toolbutton, widget);
        QRect                  ir    = toolbutton->rect;
        QStyleOptionToolButton newBtn= *toolbutton;
        newBtn.rect                  = QRect (ir.right () + 5 - mbi,
                                              ir.y () + ir.height () - mbi + 4, mbi - 6, mbi - 6);
...
QSize
QTMStyle::sizeFromContents (ContentsType type, const QStyleOption* option,
                            const QSize&   contentsSize,
                            const QWidget* widget) const {
...
int
QTMStyle::pixelMetric (PixelMetric metric, const QStyleOption* opt,
                       const QWidget* widget) const {
  switch (metric) {
...
QTMStyle::drawControl (ControlElement element, const QStyleOption* option,
  ...
  case CE_ToolBar: {
    ...
    if (use_unified_toolbar && (widget) &&
        (widget->windowTitle () == "mode toolbar")) {
      ...
      if (option->state & State_Horizontal)
        linearGrad=
            QLinearGradient (0, option->rect.top (), 0, option->rect.bottom ());
      else
        linearGrad=
            QLinearGradient (option->rect.left (), 0, option->rect.right (), 0);
  ...
  case CE_ToolBar:
#ifdef UNIFIED_TOOLBAR
    if (use_unified_toolbar && (widget) &&
        (widget->windowTitle () == "main toolbar"))
      break;
#endif // UNIFIED_TOOLBAR
    ...
    else baseStyle ()->drawControl (element, option, painter, widget);
    break;
#endif
...
int
QTMStyle::styleHint (StyleHint hint, const QStyleOption* option,
                     const QWidget*    widget,
                     QStyleHintReturn* returnData) const {
  switch (hint) {
  case SH_MenuBar_AltKeyNavigation:
    return 0;
    // Set SH_MenuBar_AltKeyNavigation to false. Typically this would be the job
    // of the style that is selected. However: That mechanism seems to be broken
    // with some Qt versions. Furthermore, the Alt key is heavily used within
    // TeXmacs, so the menubar navigation gets in the way quite often.
  default:
    return baseStyle ()->styleHint (hint, option, widget, returnData);
  }
}
