...
void
qt_image_to_eps (url image, url outfile, int w_pt, int h_pt, int dpi) {
...
qt_image_to_eps (url image, url eps, int w_pt, int h_pt, int dpi) {
  if (DEBUG_CONVERT) debug_convert << "qt_image_to_eps " << image << " ->
"<<eps<<LF; string r= qt_image_to_eps (image, w_pt, h_pt, dpi); save_string
(eps, r);
}
...
string
from_key_press_event (QKeyEvent event) {
  int                   key = event->key ();
...
string
from_key_release_event (QKeyEvent event) {
  string r;
