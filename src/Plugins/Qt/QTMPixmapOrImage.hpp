...
struct QTMPixmapOrImage {
  ...
  QTMPixmapOrImage (int w, int h) {
    if (headless_mode) rep= (void*) new QImage (w, h, QImage::Format_ARGB32);
    else rep= (void*) new QPixmap (w, h);
  }
  QTMPixmapOrImage (QSize s) {
    if (headless_mode) rep= (void*) new QImage (s, QImage::Format_ARGB32);
    else rep= (void*) new QPixmap (s);
  }
  ...
  QTMPixmapOrImage (const QTMPixmapOrImage& pxim) {
    ...
    if (headless_mode) *((QImage*) rep)= *((QImage*) pxim.rep);
    else *((QPixmap*) rep)= *((QPixmap*) pxim.rep);
  }
  ...
  void fill (const QColor& c) {
    if (headless_mode) ((QImage*) rep)->fill (c);
    else ((QPixmap*) rep)->fill (c);
  }
  bool isNull () {
    return headless_mode ? ((QImage*) rep)->isNull ()
                         : ((QPixmap*) rep)->isNull ();
  }
  QImage* QImage_ptr () {
