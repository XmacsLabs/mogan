...
QTMPrinterSettings::QTMPrinterSettings ()
    : collateCopies (true), blackWhite (true), printerName (""), fileName (""),
      paperSize ("A4"), dpi (600), firstPage (0), lastPage (0),
      printOddPages (true), printEvenPages (true), fitToPage (true),
      copyCount (1), duplex (false), pagesPerSide (1), pagesOrder (LR_TB),
      orientation (Portrait) {
  configProgram= new QProcess (this);

  // See the class documentation for an explanation of the connected signal
  QObject::connect (configProgram,
                    SIGNAL (finished (int, QProcess::ExitStatus)), this,
                    SLOT (systemCommandFinished (int, QProcess::ExitStatus)));
}
...
#if QT_VERSION >= 0x060000
  to.setPageOrientation ((orientation == Landscape) ? QPageLayout::Landscape
...
#define PAPER(fmt)                                                             \
  if (_size == "fmt") return QPageSize::fmt
  PAPER (A0);
  PAPER (A1);
  PAPER (A2);
  PAPER (A3);
...
CupsQTMPrinterSettings::fromSystemConfig (const QString& printer) {
  if (configProgram->state () != QProcess::NotRunning) return false;
 ...
 */
void
CupsQTMPrinterSettings::systemCommandFinished (
...
CupsQTMPrinterSettings::toSystemCommand () const {
  ...
  else if (!printProgram.isEmpty ()) _cmd+= printProgram;

  if (!printerName.isEmpty ()) _cmd+= QString (" -d \"%1\"").arg (printerName);
  ...
  if (firstPage != 0 || lastPage != 0) {
    int f= (int) floor (firstPage / pagesPerSide);
    f    = (f == 0) ? 1 : f;
    int l= (int) ceil (lastPage / pagesPerSide);
    l    = (l == 0) ? 1 : l;
    if (firstPage > lastPage) _cmd+= " -o outputorder=reverse";
  ...
  if (!printOddPages) _cmd+= " -o page-set=even";
  else if (!printEvenPages) _cmd+= " -o page-set=odd";
...
CupsQTMPrinterSettings::availablePrinters () {
  ...
  foreach (QString _line, _lines) {
    ...
    if (!match.hasMatch ()) continue;
    _ret.append (
        QPair<QString, QString> (match.captured (1), match.captured (1)));
  }
  return _ret;
#else
  QRegExp rx ("^(\\S+) +.*$");
...
WinQTMPrinterSettings::fromSystemConfig (const QString& printer) {
  ...
  configProgram->start (
      QString ("winprinfo --printer=\"\"\"%1\"\"\"").arg (printer));
  printerName= printer;
  return true;
}
...
WinQTMPrinterSettings::toSystemCommand () const {
  ...
  if (!printProgram.isEmpty ()) _cmd+= printProgram;
  ...
  */
  ...
  if (pagesPerSide > 1) _cmd+= QString (" -copies %1").arg (pagesPerSide);
  ...
  if (copyCount > 1) _cmd+= QString (" -copies %1").arg (pagesPerSide);

  if (!printOddPages) _cmd+= " -even";
  else if (!printEvenPages) _cmd+= " -odd";
  ...
  foreach (QString _line, _lines) {
    ...
    if (readingSizes) {
      QRegularExpression rx2 (
          "^.*mm *(\\w)+.*$"); // [ 0]   215.90  279.40 mm  Letter
      rx2.setPatternOptions (QRegularExpression::InvertedGreedinessOption);
      QRegularExpressionMatchIterator rx2_it= rx2.globalMatch (_line);
      if (rx2_it.hasNext ()) {
        QRegularExpressionMatch match= rx2_it.next ();
...
WinQTMPrinterSettings::availablePrinters () {
  ...
  foreach (QPrinterInfo printer, QPrinterInfo::availablePrinters ())
    _ret << QPair<QString, QString> (printer.printerName (),
                                     printer.printerName ());
  return _ret;
}
