...
void GuestNotificationBar::setupUI() {
  ...
  m_messageLabel->setText(qt_translate("You are currently in guest mode, some features are limited"));
  ...
  qDebug() << "Close button added to main layout, visible:" << m_closeButton->isVisible();

  // 连接信号
  connect(m_loginButton, &QPushButton::clicked, this, &GuestNotificationBar::loginRequested);
  connect(m_closeButton, &QPushButton::clicked, this, &GuestNotificationBar::closeRequested);
...
void GuestNotificationBar::applyStyle() {
...
void GuestNotificationBar::setMessage(const QString& message) {
  m_messageLabel->setText(qt_translate(from_qstring(message)));
}

QString GuestNotificationBar::message() const {
  return m_messageLabel->text();
}

void GuestNotificationBar::setLoginButtonVisible(bool visible) {
  m_loginButton->setVisible(visible);
}

bool GuestNotificationBar::isLoginButtonVisible() const {
  return m_loginButton->isVisible();
}

void GuestNotificationBar::setCloseButtonVisible(bool visible) {
  m_closeButton->setVisible(visible);
}

bool GuestNotificationBar::isCloseButtonVisible() const {
  return m_closeButton->isVisible();
}

QPushButton* GuestNotificationBar::loginButton() const {
  return m_loginButton;
}

QPushButton* GuestNotificationBar::closeButton() const {
