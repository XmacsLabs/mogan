...
extract_display (tree t, string s) {
  tree r= tree ();
  if (is_func (t, NUMBER, 2) && as_string (t[0]) == s) return t;
  else if (!is_atomic (t)) {
    for (int i= 0; i < N (t); i++) {
...
extract_class_body (tree& body, tree doc) {
  ...
  else if (is_func (doc, WITH)) extract_class_body (body, doc[N (doc) - 1]);
  else if (is_compound (doc)) body << doc;
}
...
filter_theorem (tree doc) {
  ...
  tm_std_remarks ("Assumption")     = "assumption";
  ...
  for (int j= 0; j < N (sectional_macros); j++)
    for (int i= 0; i < N (doc); i++)
      if (is_func (doc[i], ASSIGN, 2))
        if (doc[i][0] == sectional_macros[j]) r << doc[i][1][0];
  return r;
}
...
filter_setlength (tree doc) {
  ...
  array<tree> r                       = array<tree> ();
  global_layout_parameters << "tex-odd-side-margin"
  ...
  for (int j= 0; j < N (global_layout_parameters); j++)
    for (int i= 0; i < N (doc); i++)
      if (is_func (doc[i], ASSIGN, 2))
        if (doc[i][0] == global_layout_parameters[j]) r << doc[i];
  return r;
}
...
filter_custom_names (tree doc) {
  ...
  array<tree> r           = array<tree> ();
  custom_names << "abstract-text"
               << "appendix-text"
               << "table-of-contents-text"
               << "figure-text"
               << "index-text"
               << "list-of-figures-text"
               << "list-of-tables-text"
               << "part-text"
               << "bibliography-text";
  for (int j= 0; j < N (custom_names); j++)
    for (int i= 0; i < N (doc); i++)
      if (is_func (doc[i], ASSIGN, 2))
        if (doc[i][0] == custom_names[j]) r << doc[i];
  return r;
}
...
filter_enumerate (tree doc) {
  ...
  if (maxlevel > 0) {
    maxlevel= min (maxlevel, 4);
    r << tree (ASSIGN, "enumerate-reduce",
               tree (MACRO, "nr",
                     tree (MINIMUM, tree (ARG, "nr"), as_string (maxlevel))));
  }
  return r;
...
latex_class_filter (tree t) {
  ...
  */
  ...
  doc << source_comment (header) << incls
