...
get_latex_errors (url log) {
  ...
  while (i < n) {
    while (i < n) {
      if (i + 1 < n && s[i] == '!' && s[i + 1] == ' ')
        if (i == 0 || s[i - 1] == '\12' || s[i - 1] == '\15') break;
      i++;
    }
    if (i < n) {
      int start= i;
...
texmacs_error_find (string s, string src, hashmap<int, array<path>> corr) {
  ...
  while (pos > 0) {
    if (corr->contains (pos)) {
      ...
      for (int i= 0; i < N (a); i++)
        if (!is_nil (a[i]) && last_item (a[i]) == 0) return path_up (a[i]);
    }
    pos--;
  }
  return path ();
...
try_latex_export (tree doc, object opts, url src, url dest) {
  ...
  string cmd= "cd " * sys_concretize (head (dest)) *
              "; pdflatex -interaction=batchmode " *
              as_system_string (tail (dest));
  // cout << cmd << LF;
  ...
  for (int i= 0; i < N (errs); i++) {
    ...
    */
    ...
    tree t (TUPLE);
    t << tree (err)
      << tree ("l." * latex_error_line (err) * " " * latex_error_message (err))
      << tree (latex_error_message (err)) << tree (latex_error_explain (err))
      << tree (latex_error_position (err)) << tree (latex_error_extra (err));
