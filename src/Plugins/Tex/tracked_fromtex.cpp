...
latex_protect (string s, hashset<int>& l) {
  ...
  for (i= 0; i < n;)
    if (s[i] == '\\') {
      if (test (s, i, "\\begin{verbatim}")) latex_protect (s, l, i, "verbatim");
      else if (test (s, i, "\\begin{alltt}")) latex_protect (s, l, i, "alltt");
      else if (test (s, i, "\\begin{tmcode}")) latex_protect (s, l, i, "alltt");
      else i++;
    }
    else i++;
}
...
texmacs_group_markers (tree t) {
  if (is_atomic (t)) return t;
    ...
    if (is_concat (r) && N (r) >= 2 && is_compound (r[0], "blx", 1) &&
    ...
    if (is_document (r) && N (r) >= 2 && is_compound (r[0], "blx", 1) &&
        is_compound (r[N (r) - 1], "elx", 1)) {
      string b= as_string (r[0][0]);
      string e= as_string (r[N (r) - 1][0]);
      r       = r (1, N (r) - 1);
...
texmacs_correct_markers (tree t, int b, int e) {
  if (is_atomic (t)) return t;
    ...
    int sb, se;
    get_range (t[0], sb, se);
    if (sb <= b || se >= e) return texmacs_correct_markers (t[1], b, e);
...
tree
texmacs_unmark (tree t) {
  return texmacs_unmark (t, true);
}
tree
texmacs_clean_markers (tree t) {
  return texmacs_unmark (t, false);
}
...
static void
find_matches (tree d1, int b1, int e1, tree d2, int b2, int e2, array<int>& c) {
...
texmacs_declare_opaque (tree mt, path p, hashset<int>& invalid) {
  ...
  if (!is_nil (p)) texmacs_declare_opaque (mt[p->item], p->next, invalid);
}
...
texmacs_check_transparency (tree mt, tree t, hashset<int>& invalid) {
  ...
  if (is_compound (mt, "mlx", 2)) {
    ...
    while (delta < N (umt)) {
      for (int i= 0; i < N (umt); i++)
        if (c[i] < 0) {
          // cout << "Make opaque: " << a[i] << ", " << subtree (mt, a[i]) <<
          // LF;
          texmacs_declare_opaque (mt, a[i], invalid);
          if (delta > 0) {
            tree st= subtree (mt, a[i]);
            if (is_compound (st) && N (st) == 1 && is_document (st[0]) &&
                N (st[0]) == 1 && is_compound (st[0][0], "mlx", 2))
      ...
      while (i < N (umt)) {
        ...
        while (i < N (umt) && c[i] < 0)
          i++;
        if (i != N (umt)) i++;
        for (int j= max (0, start - delta); j < start; j++)
          c[i]= -1;
        for (int j= i; j <= min (N (umt), i + delta); j++)
          c[i]= -1;
        i= min (N (umt), i + delta);
      }
      delta*= 2;
