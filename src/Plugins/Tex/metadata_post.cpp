...
cleanup (tree t) {
  ...
  for (int i= 0; i < N (t); i++)
    ...
    else if (is_func (t[i], WITH, 3) && t[i][0] == FONT_FAMILY &&
             t[i][1] == "tt") {
      tree c= copy (t[i]);
      tree b= cleanup (c[2]);
      if (is_concat (b)) {
...
explode_affiliation (tree t, array<tree>& affs) {
  ...
  if (is_concat (t)) {
    ...
    while (i < n) {
      tree aff (CONCAT);
      while (i < n) {
        if ((L (t[i]) == NEXT_LINE || L (t[i]) == NEW_LINE) && i + 1 < n &&
            is_reference (t[i + 1], false))
          break;
        else aff << t[i++];
      }
...
add_author (array<tree>& auts, tree& aut) {
  if (N (aut) == 0) return;
  if (N (auts) == 0 || has_lowercase (aut)) auts << simplify_concat (aut);
  else {
...
maximal_clustering (tree t) {
  for (int i= 0; i < N (t); i++)
    if (is_compound (t[i], "doc-title-options")) return t;
  tree r= copy (t);
    ...
    if (is_author_with_name (t[i])) {
      for (int j= 0; j < N (t[i][0]); j++)
        if (is_compound (t[i][0][j], "author-name")) {
          ...
          if (i + 1 < N (t) && is_compound (t[i + 1], "doc-author", 1) &&
...
expand_emails (tree t) {
  ...
  for (int i= 0; i < N (t); i++)
    if (is_compound (t[i], "doc-author", 1) &&
      ...
      tree          v (L (u));
      for (int j= 0; j < N (u); j++)
        if (is_compound (u[j], "author-email", 1) && is_atomic (u[j][0])) {
          ...
          for (int k= 0; k < N (b); k++)
            while (ends (b[k], ",") || ends (b[k], ")") || ends (b[k], ".") ||
                   ends (b[k], " "))
              b[k]= b[k](0, N (b[k]) - 1);
...
fetch_names (tree t, array<string>& names) {
  if (is_atomic (t)) {
    ...
    for (int i= 0; i < N (l); i++)
      if (uni_upcase_all (l[i]) != l[i] && uni_locase_all (l[i]) != l[i] &&
          N (l[i]) > 3)
        names << l[i];
  }
  else if (is_concat (t)) {
...
clean_emails (tree t) {
  tree r (L (t));
  for (int i= 0; i < N (t); i++)
    if (t[i] != compound ("doc-author", compound ("author-data"))) r << t[i];
  return r;
}
  ...
  for (int i= 0; i < N (t);)
    if (is_author_with_name (t[i])) {
      ...
      for (k= j; k < N (t); k++)
        ...
        else if (is_author_without_name (t[k])) fetch_emails (t[k][0], emails);
          ...
          for (int l= 0; l < N (inds); l++)
            for (int m= 0; m < N (inds); m++)
              if (!done[l] && !done[m] && m != l)
                if (match_author_email (t[inds[l]], emails[m])) ok= false;
        }
        if (ok) {
          if (N (inds) > N (emails))
...
attach_pending (tree orig) {
  ...
  tree r (L (t));
  for (int i= 0; i < N (t); i++)
    if (!is_author_without_name (t[i])) r << t[i];
  if (cluster) r= maximal_clustering (r);
  return r;
}
