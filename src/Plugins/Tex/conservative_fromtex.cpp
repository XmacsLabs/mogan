...
texmacs_merge_preamble (tree newt, tree oldt) {
  ...
  hashmap<string,tree> oldi= tree_hashmap<string> (as_string (UNINIT), extract (oldt, "initial"));
  hashmap<string,tree> newi= tree_hashmap<string> (as_string (UNINIT), extract (newt, "initial"));
  oldi->join (newi);
