...
revtex_contains_affiliation (tree t) {
  for (int i= 0; i < N (t); i++)
    if (is_apply (t[i], "\\author-affiliation")) return true;
  return false;
}
...
revtex_get_affiliations (tree t) {
  array<tree> r;
  for (int i= 0; i < N (t); i++)
    if (is_apply (t[i], "\\author-affiliation")) r << t[i];
  return r;
}
...
collect_metadata_revtex (tree t) {
  ...
  for (i= 0; i < n; i++) {
    ...
    // doc datas
    ...
    else if ((is_tuple (u, "\\thanks", 1) || is_tuple (u, "\\tmnote", 1)) &&
             !author)
      ...
      doc_data << tree (APPLY, "\\doc-note", concat ("Copyright year: ", u[1]));
    ...
    // authors datas
      ...
      author= true;
      if (N (author_data) > 1) authors << author_data;
