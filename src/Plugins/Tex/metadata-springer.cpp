...
get_author_name (tree t) {
  for (int i= 0; i < N (t); i++)
    if (is_apply (t[i], "\\author-name")) return t[i];
  return concat ();
}
...
merge_llncs_author_datas (array<tree> authors, array<tree> affs) {
  ...
  for (int i= 0; i < N (authors); i++)
    for (int j= 1; j < N (authors[i]); j++)
      if (is_apply (authors[i][j], "\\author-inst", 1)) simple= false;

  array<tree> r;
  for (int i= 0; i < N (authors); i++) {
    if (simple) r << add_llncs_author_datas (authors[i], affs);
    else {
      array<tree> author_affs;
      for (int j= 1; j < N (authors[i]); j++) {
        if (is_apply (authors[i][j], "\\author-inst", 1)) {
          int n= as_int (as_string (authors[i][j][1]));
          if (n > 0 && n <= N (affs)) author_affs << affs[n - 1];
        }
      }
...
merge_springer_author_datas (array<tree> a) {
  ...
  for (int i= 1; i < N (same); i++)
    for (int j= 0; j < N (same[i]); j++)
      if (!contains (same[i][j], A (datas))) datas << same[i][j];
  array<tree> r;
  r << datas;
...
clean_springer_title_markup (tree t) {
  ...
  if (is_springer_titlenote (t) || is_tuple (t, "\\tmmisc", 1))
    return concat ();
  tree r (L (t));
  int  i, n= N (t);
  for (i= 0; i < n; i++)
      ...
      if (N (author_name) > 1) {
        ...
        for (int j= 0; j < N (author_name); j++)
          if (j + 1 < N (author_name) || !is_line_break (author_name[j]))
            tmp << author_name[j];
        if (N (tmp) > 1) author_data << tree (APPLY, s, tmp);
...
get_springer_affiliation_datas (tree t, bool llncs= false) {
  ...
  tree        tmp (CONCAT);
  for (i= 0; i < n; i++) {
    while (i < n && !is_tuple (t[i], "\\at"))
      tmp << t[i++];
    author_datas= get_springer_author_datas (tmp, "name");
    tmp         = concat ();
    if (is_tuple (t[i], "\\at")) {
      i++;
      while (i < n && !is_tuple (t[i], "\\and"))
        tmp << t[i++];
