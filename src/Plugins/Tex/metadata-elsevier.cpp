...
clean_elsevier_notes_refs (tree t) {
  ...
  int  i, n= N (t);
  for (i= 0; i < n; i++) {
    if (!is_elsevier_note_ref (t[i])) r << clean_elsevier_notes_refs (t[i]);
  }
  return r;
}
...
get_elsevier_author_refs (tree u) {
  array<string> r;
  array<tree>   t;
  if (is_tuple (u, "\\author*", 2)) r << tokenize (string_arg (u[1]), ",");
  u= u[N (u) - 1];
...
translate_author_metadata_elsevier (tree u) {
  if (!is_tuple (u) || N (u) < 2) return concat ();
