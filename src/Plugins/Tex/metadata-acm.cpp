...
clean_acm_title_markup (tree t) {
  ...
  if (is_acm_titlenote (t) || is_tuple (t, "\\tmacmmisc", 1) ||
      is_tuple (t, "\\tmacmsubtitle", 1))
    return concat ();
  if (is_tuple (t, "\\ttlit")) {
    t[0]= "\\it";
    return t;
      ...
      if (N (author_name) > 1) {
        ...
        for (int j= 0; j < N (author_name); j++)
          if (j + 1 < N (author_name) || !is_line_break (author_name[j]))
            tmp << author_name[j];
        if (N (tmp) > 1) author_data << tree (APPLY, "\\author-name", tmp);
...
collect_metadata_acm_old (tree t) {
  ...
  for (i= 0; i < n; i++) {
    u= t[i];
    if (is_tuple (u, "\\date", 1)) doc_data << tree (APPLY, "\\doc-date", u[1]);
      ...
      doc_data << tree (
          APPLY, "\\doc-author",
          tree (APPLY, "\\author-data", tree (APPLY, "\\author-name", u[1])));
    }
      ...
      while (i < n && !is_tuple (t[i], "\\end-keywords"))
        keywords << t[i++];
      array<tree> tmp= tokenize_concat (
          keywords, A (concat (",", ";", tree (TUPLE, "\\tmsep"),
                               tree (TUPLE, "\\tmSep"))));
      if (N (tmp) > 0) {
      ...
      array<tree> tmp= tokenize_concat (
          u[N (u) - 1], A (concat (",", ";", tree (TUPLE, "\\tmsep"),
                                   tree (TUPLE, "\\tmSep"))));
      if (N (tmp) > 0) {
        tree kw= tree (APPLY, "\\abstract-keywords");
        kw << tmp;
...
get_acm_affiliation (tree orig) {
  ...
  for (int i= 0; i < N (t); i++)
    if (is_tuple (t[i], "\\institution", 1) ||
        is_tuple (t[i], "\\streetaddress", 1) || is_tuple (t[i], "\\city", 1) ||
        is_tuple (t[i], "\\state", 1) || is_tuple (t[i], "\\postcode", 1) ||
  ...
  for (i= 0; i < n; i++) {
    ...
    if (is_tuple (u, "\\date", 1)) doc_data << tree (APPLY, "\\doc-date", u[1]);
      ...
      while (i < n) {
        ...
        else if (is_tuple (t[i], "\\orcid", 1)) i++;
        ...
        else if (is_atomic (t[i]) && is_whitespace (t[i]->label)) i++;
        else break;
      }
