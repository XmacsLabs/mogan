...
void
load_entities (hashmap<string, string> table, string fname) {
  string s;
  if (DEBUG_CONVERT) debug_convert << "Loading " << fname << "\n";
  if (load_string (url ("$TEXMACS_PATH/langs/encoding", fname), s, false))
    return;
...
xml_html_parser::parse_entity () {
  ...
  if (test (s, "#")) {
    ...
    if (test (s, "x") || test (s, "X")) {
      r << s->read (1);
      while (s && is_hex_digit (s[0]))
        r << s->read (1);
    }
    else
      while (s && is_digit (s[0]))
        r << s->read (1);
  }
  else
    while (s && is_name_char (s[0]))
...
xml_html_parser::parse_attribute () {
  ...
  if (test (s, "=")) s+= 1;
  skip_space ();
  if (test (s, "\42") || test (s, "'")) val= parse_quoted ();
  else { // for Html
    string r;
    while (s) {
...
xml_html_parser::build_valid_child (string parent, string child) {
  ...
  if (child == "li")
    return (parent == "ul") || (parent == "ol") || (parent == "dir") ||
           (parent == "menu");
  ...
  if (child == "tr")
    return (parent == "table") || (parent == "thead") || (parent == "tfoot") ||
           (parent == "tbody");
  if ((child == "th") || (child == "td"))
    return (parent == "tr") || (parent == "table") || (parent == "thead") ||
...
xml_html_parser::build (tree& r) {
  while (i < n) {
    if (is_tuple (a[i], "begin")) {
      ...
      sub[0]  = "tag";
      i++;
      if (html && html_empty_tag_table->contains (name)) r << sub;
      else {
...
xml_html_parser::finalize_space (string s, bool first, bool last) {
  ...
  n= N (r);
  if (last && (n > 0) && (r[n - 1] == ' ')) r->resize (n - 1);
  return r;
}
...
xml_html_parser::finalize_space (tree t) {
  if (is_atomic (t) || (!is_tuple (t, "tag"))) return t;
    ...
    if (!is_tuple (t[n - 1], "attr")) last= n - 1;
    (void) first;
    (void) last;
    for (i= 2; i < n; i++) {
      if (is_atomic (t[i])) {
...
xml_html_parser::finalize_sxml (tree t) {
  ...
  for (i= 2; i < n; i++)
    ...
    else if (is_tuple (t[i], "tag")) content << finalize_sxml (t[i]);
    else if (is_atomic (t[i])) content << xml_quote (t[i]->label);
    ...
    else if (is_tuple (t[i], "cdata")) content << xml_quote (t[i][1]->label);
