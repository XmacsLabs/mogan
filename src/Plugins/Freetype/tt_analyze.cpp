...
bool
sane_font (array<string> r, font_metric fnm, string family, string range,
           int test) {
...
analyze_range (font fn, font_metric fnm, array<string>& r, string family) {
  ...
  if (starts (lfn, "lastresort") || starts (lfn, "tex ams blackboard bold") ||
      (starts (lfn, "tex blackboard bold") &&
       !starts (lfn, "tex blackboard bold variant")) ||
      starts (lfn, "tex calligraphic capitals") ||
      starts (lfn, "tex double stroke") || starts (lfn, "tex ralph smith"))
    return;
  if (range_exists (fnm, 0x21, 0x7e) || ec) r << string ("Ascii");
  if (range_exists (fnm, 0xc0, 0xff) || ec) r << string ("Latin");
  if (range_percentage (fnm, 0x391, 0x3ce) > 66.6 || ec) r << string ("Greek");
...
irregularity (font_metric fnm) {
  ...
  array<int>     xlike;
  xlike << 0x61 << 0x63 << 0x65 << 0x6d << 0x6e << 0x6f << 0x75 << 0x76 << 0x77;
  int totdy= 0;
...
analyze_trace (font fn, font_metric fnm, array<string>& r) {
  if (range_exists (fnm, 0x41, 0x5a)) {
    ...
    r << (string ("upw=") * wt);
    array<int> ha;
    ha << 0x41 << 0x42 << 0x43 << 0x44 << 0x47 << 0x48 << 0x4a << 0x4d << 0x4e
       << 0x51;
    string ht= height_trace (fnm, ha);
    r << (string ("uph=") * ht);
    array<int> ca;
    ca << 0x41 << 0x42 << 0x43 << 0x44 << 0x45 << 0x48 << 0x49 << 0x4a << 0x4d
       << 0x4e << 0x53 << 0x57;
  ...
  if (range_exists (fnm, 0x61, 0x7a)) {
    ...
    r << (string ("low=") * wt);
    array<int> ha;
    ha << 0x61 << 0x62 << 0x63 << 0x64 << 0x66 << 0x67 << 0x69 << 0x6a << 0x70
       << 0x71 << 0x74 << 0x7a;
    string ht= height_trace (fnm, ha);
    r << (string ("loh=") * ht);
    array<int> ca;
    ca << 0x61 << 0x62 << 0x63 << 0x64 << 0x65 << 0x66 << 0x68 << 0x69 << 0x6d
       << 0x72 << 0x79 << 0x7a;
    string ct= count_trace (fn, ca);
...
find_attribute_value (array<string> a, string s) {
  string s2= s * "=";
  for (int i= 0; i < N (a); i++)
    if (starts (a[i], s2)) return a[i](N (s2), N (a[i]));
  return "";
}
...
trace_distance (string v1, string v2, double m) {
  ...
  for (int i= 0; i < N (v1); i++) {
    double x=
        db_abs (((double) (int) (v1[i])) - ((double) (int) (v2[i]))) / 9.0;
    d+= x * x;
  }
...
characteristic_distance (array<string> a1, array<string> a2) {
  ...
  double r= d_mono + d_sans + d_italic + d_case + d_ex + d_em + d_lvw + d_lhw +
            d_fillp + d_vcnt + d_lasprat + d_pasprat + d_loasc + d_lodes +
            d_dides + d_slant;
