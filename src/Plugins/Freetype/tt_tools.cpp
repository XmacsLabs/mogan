...
url
tt_unpack (string s) {
  // s must be a basename with a int suffix, like basename.1
  if (!is_int (suffix (url (s)))) return url_none ();

  // check if the unpacked font exists
  url dir= url ("$TEXMACS_HOME_PATH/fonts/unpacked");
  if (!exists (dir)) mkdir (dir);
  url sub_font= dir * url (s * ".ttf");
  if (exists (sub_font)) return sub_font;
  ...
  if (is_none (main_font)) {
    debug_fonts << "Failed to find the main font: " << s << LF;
    return url_none ();
  }

  // Load the main font or just return url_none
  string ttc;
  if (load_string (main_font, ttc, false)) {
    debug_fonts << "Failed to load the main font:" << LF << main_font << LF;
    return url_none ();
  }

  // Extract the subfont or just return url_none
  string tt= tt_extract_subfont (ttc, i);
  if (save_string (sub_font, tt, false)) {
    debug_fonts << "Failed to save the sub font:" << LF << sub_font << LF;
    return url_none ();
  }

  // return the url of the sub font
  return sub_font;
}
