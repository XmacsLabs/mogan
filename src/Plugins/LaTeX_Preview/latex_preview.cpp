...
search_latex_previews (tree t) {
  ...
  if (is_atomic (t))
    ;
           ...
           is_tuple (t, "\\newenvironment*"))
    ;
  else if (is_tuple (t, "\\latex_preview", 2)) r << as_string (t[1]);
  else {
    int i, n= N (t);
    for (i= 0; i < n; i++)
...
latex_clean_tmp_directory (url u) {
  ...
  for (int i= 0; i < N (content); i++)
    if (content[i] != "." && content[i] != "..") remove (u * content[i]);
  rmdir (u);
}
...
latex_remove_fmt (string s) {
  ...
  while (i < n) {
    ...
    else if (test (s, i, "\\usepackage") || test (s, i, "\\begin")) break;
    i++;
  }
  return s;
}
...
latex_install_preview (string s, tree t, url wdir, bool dvips) {
  ...
  if (!dvips) preview << "\\usepackage[active,tightpage,delayed]{preview}\n";
    ...
    preview
        << "\\usepackage[active,tightpage,delayed,psfixbb,dvips]{preview}\n";
...
latex_load_preview (url wdir, bool dvips= false) {
  ...
  if (dvips) {
    cmdln << "dvips temp.dvi && "
          << gs_prefix () * " -sDEVICE=" * eps_device () *
                 " -dSAFER -q -dNOPAUSE -dBATCH "
          << "-dLanguageLevel=3 -sOutputFile=temp%d.eps temp.ps";
  }
  else {
    cmdln << gs_prefix () * " -sDEVICE=" * eps_device () *
                 " -dSAFER -q -dNOPAUSE -dBATCH "
  ...
  while (!stop) {
    ...
    if (exists (u)) {
      ...
      if (N (tmp) == 5 && (tmp[1] != "0pt" || tmp[2] != "0pt")) r << tmp;
      cnt++;
    }
    else stop= true;
  }
