...
database_rep::line_satisfies (db_line_nr nr, db_constraint c, db_time t) {
  ...
  // cout << "    Testing " << from_atom (l->id) << ", " << from_atom (l->attr)
  // << ", " << from_atom (l->val) << LF;
...
database_rep::encode_constraint (tree q) {
  ...
  if (attr == "any") r << -1;
  else if (attr == "keywords") return encode_keywords_constraint (q);
  else if (attr == "order") {
    r << -2;
    return r;
  }
  ...
  else if (!is_quoted (q[0]->label)) return db_constraint ();
...
database_rep::query (tree ql, db_time t, int limit) {
  ...
  for (int i= 0; i < N (ql); i++) {
    if (is_tuple (ql[i], "modified", 2) && is_atomic (ql[i][1]) &&
        is_atomic (ql[i][2]) && is_quoted (ql[i][1]->label) &&
        is_quoted (ql[i][2]->label)) {
