...
is_gbt7714_style () {
  ...
  tmscm result= eval_scheme ("(or (bib-mode? \"gbt7714-2015\") (bib-mode? \"gbt7714-2015-author-year\"))");
...
bib_to_latex (string s) {
  ...
  while (pos < N (s)) {
    ...
    // convert_warning << ch << " " << as_string (keepcase) << " " << as_string
    // (depth) << "\n";
    if (ch == "$$") r << "$";
...
bib_change_first (tree& t, string (*change_fun) (string)) {
  ...
  else if (is_compound (t, "verbatim")) return false;
  else if (is_compound (t, "slink")) return false;
  else if (is_func (t, WITH, 3) && t[0] == FONT_FAMILY && t[1] == "tt")
    return false;
  else if (is_func (t, WITH, 3) && t[0] == MATH_FONT) return false;
  else if (is_compound (t, "math")) return false;
  else {
    int pos= 0;
    if (L (t) == WITH) pos= N (t) - 1;
    else
      while ((pos < N (t)) && bib_is_bad (t[pos]))
        pos++;
...
bib_change_case (tree& t, string (*change_case) (string)) {
  ...
  if (is_atomic (t) && change_case) t->label= change_case (t->label);
    ...
    for (int i= 0; i < N (t); i++)
      bib_change_case (t[i], change_case);
}
...
search_and_keyword (string s, int& pos) {
  ...
  while (pos < N (s)) {
    ...
    if (bib_is_space (ch) && depth == 0) {
      while (pos < N (s) && is_space (s[pos]))
        pos++;
      if (pos < N (s) - 3 && s (pos, pos + 3) == "and" &&
          is_space (s[pos + 3])) {
        pos+= 4;
        return true;
      }
...
static bool
bib_is_latex (tree t) {
  return as_string (L (t)) == "bib-latex" && arity (t) == 1 && is_atomic (t[0]);
}
...
bib_assoc (tree entry, string key) {
  if (bib_is_entry (entry)) {
    ...
    for (int i= 0; i < N (doc); i++)
      if (bib_is_field (doc[i]) && doc[i][0]->label == key) return doc[i][1];
  }
  return "";
}
...
bib_get_fields (tree t, string& latex) {
  ...
  for (int i= 0; i < N (t); i++) {
    ...
    else if (bib_is_comment (t[i])) nbfields+= bib_get_fields (t[i], latex);
  }
  return nbfields;
}
...
bib_set_fields (tree& t, array<tree> latex, int& ind) {
  for (int i= 0; i < N (t); i++) {
    if (bib_is_entry (t[i])) {
      ...
      for (int j= 0; j < N (t[i][2]); j++) {
        ...
        if (bib_is_field (f)) {
          if ((f[0]->label == "author" || f[0]->label == "editor") &&
              is_atomic (f[1])) {
            tree res= compound ("bib-names");
            ind++;
            while (latex[ind] == compound ("bibname")) {
              tree name= compound ("bib-name");
              ind++;
              name << latex[ind];
              ind++;
              name << latex[ind];
    ...
    else if (bib_is_comment (t[i])) bib_set_fields (t[i], latex, ind);
  }
}
...
bib_parse_fields (tree& t) {
  ...
  if (N (latex) == nb - 1 && ends (fields, "\\nextbib{}")) {
    ...
    fields << "{xyzyx}";
    latex= bib_latex_array (latex_to_tree (
        parse_latex (cork_to_sourcecode (fields), false, false)));
  ...
  if (L (t) == DOCUMENT) {
    ...
    for (int i= 0; i < N (str); i++) {
      if (bib_is_assign (str[i])) {
        ...
        else if (is_atomic (val)) dict (key)= val->label;
      }
    }
  }
  return dict;
...
bib_select_entries (tree t, tree bib_t) {
  ...
  for (int i= 0; i < arity (bt); i++) {
    ...
    if (!h->contains (b)) bibtex_warning << "Missing reference '" << b << "'\n";
