...
bib_skip (string s, int& i) {
  ...
  while (i < n) {
    ...
    bool   ok= (tp == "article") || (tp == "book") || (tp == "booklet") ||
             (tp == "inbook") || (tp == "incollection") ||
             (tp == "inproceedings") || (tp == "conference") ||
             (tp == "manual") || (tp == "mastersthesis") || (tp == "misc") ||
             (tp == "phdthesis") || (tp == "proceedings") ||
             (tp == "techreport") || (tp == "unpublished");
    if (ok) return true;
    bool other= (tp == "comment") || (tp == "string") || (tp == "preamble");
...
is_db_entry (tree t) {
  ...
  return is_compound (t, "db-entry") || is_compound (t, "db-folded-entry") ||
...
conservative_bib_import (string old_s, tree old_t, string new_s) {
  ...
  for (int i= 1; i < N (new_a); i+= 2) {
    if (bib_is_special (new_a[i])) delta_s << new_a[i - 1] << new_a[i];
    else {
      string key= bib_get_key (new_a[i]);
      int    j  = old_i[key];
      if (j < 0 || new_a[i] != old_a[j]) delta_s << new_a[i - 1] << new_a[i];
    }
  }
...
conservative_bib_export (string src_s, tree src_t, string obj_s, tree obj_t) {
  ...
  for (i= 1; i < n; i+= 2) {
    ...
    if (src_e->contains (key) && obj_e->contains (key) &&
...
conservative_bib_export (tree old_t, string old_s, tree new_t) {
  if (get_preference ("texmacs->bibtex:conservative", "off") != "on" ||
      !is_func (old_t, DOCUMENT) || !is_func (new_t, DOCUMENT))
  ...
  if (!bib_check_standard_preamble (old_a)) return bib_export (new_t);
  ...
  for (int i= 0; i < N (new_t); i++)
    if (is_db_entry (new_t[i])) {
      ...
      if (delta_i->contains (key)) new_s << "\n" << delta_a[delta_i[key]];
      else if (old_i->contains (key)) new_s << "\n" << old_a[old_i[key]];
    }
