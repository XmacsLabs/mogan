...
is_long (tree t) {
  if (is_compound (t)) {
    int i, n= N (t);
    for (i= 0; i < n; i++)
      if (is_long_arg (t, i)) return true;
  }
  return false;
}
...
tree
edit_env_rep::rewrite_inactive_arg (tree t, tree var, int i, bool block,
                                    bool flush) {
  tree r= subvar (var, i);
...
tree
edit_env_rep::rewrite_inactive_raw_data (tree t, tree var, bool block,
                                         bool flush) {
  (void) t;
  (void) block;
...
tree
edit_env_rep::rewrite_inactive_document (tree t, tree var, bool block,
                                         bool flush) {
  if ((block || (src_compact == COMPACT_NONE)) && (src_special > SPECIAL_RAW) &&
...
tree
edit_env_rep::rewrite_inactive_concat (tree t, tree var, bool block,
                                       bool flush) {
...
tree
edit_env_rep::rewrite_inactive_value (tree t, tree var, bool block,
                                      bool flush) {
  if ((N (t) == 1) && is_atomic (t[0]) && src_style != STYLE_SCHEME &&
...
tree
edit_env_rep::rewrite_inactive_arg (tree t, tree var, bool block, bool flush) {
  if ((N (t) == 1) && is_atomic (t[0]) && src_style != STYLE_SCHEME &&
      src_special >= SPECIAL_NORMAL) {
    tree r= highlight (subvar (var, 0), t[0],
                       inactive_mode == INACTIVE_INLINE_ERROR ||
...
tree
edit_env_rep::rewrite_inactive_symbol (tree t, tree var, bool block,
                                       bool flush) {
...
tree
edit_env_rep::rewrite_inactive_style_with (tree t, tree var, bool block,
                                           bool flush, bool once) {
  int  i, n= N (t);
  tree recover= tuple ();
  for (i= 0; i < n - 1; i+= 2)
    if (is_atomic (t[i])) {
...
tree
edit_env_rep::rewrite_inactive_active (tree t, tree var, bool block,
                                       bool flush) {
  tree st  = t[0];
...
tree
edit_env_rep::rewrite_inactive_var_active (tree t, tree var, bool block,
                                           bool flush) {
  (void) block;
  tree r= tree (WITH, MODE, copy (env[MODE]), subvar (var, 0));
  if (flush && (src_compact != COMPACT_ALL) &&
...
tree
edit_env_rep::rewrite_inactive_hybrid (tree t, tree var, bool block,
                                       bool flush) {
  if (is_atomic (t[0]) && (src_special >= SPECIAL_NORMAL)) {
    int  i, n= N (t);
    tree r (INLINE_TAG, n);
    r[0]= tree (CONCAT, "\\", highlight (subvar (var, 0), t[0], TYPE_VARIABLE));
    for (i= 1; i < n; i++)
      r[i]= rewrite_inactive_arg (t, var, i, false, false);
    return tree (MARK, var, r);
  }
...
tree
edit_env_rep::rewrite_inactive_default (tree t, tree var, bool block,
                                        bool flush) {
  int  i, d= 0, n= N (t);
  tree op= as_string (L (t));
  if ((L (t) == COMPOUND) && is_atomic (t[0]) &&
      (src_special >= SPECIAL_NORMAL)) {
    d = 1;
  ...
  if ((N (t) == d) || (src_compact == COMPACT_ALL) ||
    ...
    for (i= d; i < n; i++) {
      ...
      if (start == d) doc << u;
        ...
        if (src_close < CLOSE_LONG) doc << tree (SURROUND, "", u, next);
        else doc << next << u;
      }
    }
...
edit_env_rep::rewrite_inactive (tree t, tree var, bool block, bool flush) {
  if (is_atomic (t)) {
    if (src_style == STYLE_SCHEME)
      return tree (CONCAT, tree (WITH, COLOR, src_tag_color, "\20"), var,
