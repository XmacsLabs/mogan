  ...
  if (is_atomic (t)) return t;
    ...
    for (int i= 0; i < N (t); i++)
      if (is_tuple (t[i]) && N (t[i]) >= 2 && is_double (t[i][0])) {
        ...
        if (ti - 1.0e-6 <= tt && ti > t0) {
          i0= i;
          t0= ti;
          c0= t[i][1];
        }
        if (ti + 1.0e-6 >= tt && ti < t1) {
          i1= i;
          t1= ti;
          c1= t[i][1];
        }
      }
      else if (!is_tuple (t[i]) && i0 == -1) {
        i0= i;
        t0= 0.0;
        c0= t[i];
      }
      else if (!is_tuple (t[i]) && i1 == -1) {
        i1= i;
        t1= 1.0;
        c1= t[i];
      }
...
morph_with (tree t0, tree t1, edit_env env) {
  if (!is_func (t0, WITH)) return morph_with (tree (WITH, t0), t1, env);
  if (!is_func (t1, WITH)) return morph_with (t0, tree (WITH, t1), env);
  tree r (WITH);
...
get_anim_id (tree t) {
  ...
  for (int i= 0; i + 1 < N (t); i++)
    if (t[i] == ANIM_ID) return as_string (t[i + 1]);
...
morph_graphics (tree t0, tree t1, edit_env env) {
  ...
  */
  while (i0 < N (t0) && i1 < N (t1)) {
    ...
    if (done->contains (id0)) {
      i0++;
      continue;
    }
    if (done->contains (id1)) {
      i1++;
      continue;
    }
    if (id0 == id1 && id0 != "") {
  ...
  for (; i0 < N (t0); i0++)
    if (!done->contains (get_anim_id (t0[i0]))) r << morph (t0[i0], "", env);
  for (; i1 < N (t1); i1++)
    if (!done->contains (get_anim_id (t1[i1]))) r << morph ("", t1[i1], env);
  return r;
}

bool
morphable_arity (tree_label l) {
  return (l >= LINE && l <= CSMOOTH) || l == CONCAT || l == DOCUMENT;
}
...
complete_sub (tree t0, tree t1) {
  ...
  int l= 0, r= 0;
  while (l < n0 && t0[l] == t1[l])
    l++;
  if (l > 0) l--;
  while (r < n0 - l && t0[n0 - 1 - r] == t1[n1 - 1 - r])
    r++;
  if (r > 0) r--;
  if (l > 0 || r > 0) {
...
complete (tree t0, tree t1) {
  tree_label l= L (t0);
  if (l == CLINE || l == CARC || l == CSPLINE || l == CBEZIER || l == CSMOOTH ||
      l == ELLIPSE) {
...
morph (tree t0, tree t1, edit_env env) {
  ...
  if (is_atomic (t0) && is_atomic (t1)) {
    ...
    string s0= t0->label;
    string s1= t1->label;
    if (is_double (s0) && is_double (s1)) return morph_number (s0, s1, t);
  ...
  else if (is_func (t0, VALUE) && t1 == t0) return t0;
  ...
  else if (is_atomic (t0) || is_atomic (t1)) return morph_trivial (t0, t1, env);
...
insert_anim_ids (tree t) {
  if (is_atomic (t)) return t;
  else if (is_func (t, MORPH)) return tree (L (t), insert_anim_ids (A (t)));
  else {
    int  i, n= N (t);
    tree r (t, n);
    for (i= 0; i < n; i++) {
...
insert_frame (tree a, tree f, double t) {
  if (is_func (a, MORPH)) {
    ...
    tree r (MORPH, tree (TUPLE, "0", a), tree (TUPLE, as_string (t), f));
    return r;
  }
}
...
edit_env_rep::expand_morph (tree t) {
  if (is_func (t, ANIM_STATIC, 4) || is_func (t, ANIM_DYNAMIC, 4) ||
