...
wide_tilda_box (path ip, SI x1, SI x2, pencil pen) {
  ...
  bs[0]=
      arc_box (decorate_middle (ip), 0, -h, 2 * uw, h, 60 << 6, 180 << 6, pen);
  bs[1]= line_box (decorate_middle (ip), 3 * uw / 2, hh, w - (3 * uw / 2),
                   h - hh, pen);
  bs[2]= arc_box (decorate_middle (ip), w - (2 * uw), 0, w, 2 * h, 240 << 6,
                  360 << 6, pen);
  return composite_box (ip, bs, xs, ys);
}
...
wide_vect_box (path ip, SI x1, SI x2, pencil pen) {
  ...
  bs[1]= line_box (decorate_middle (ip), width - arrow - delta, 0, width, arrow,
                   pen);
  bs[2]= line_box (decorate_middle (ip), width + delta - arrow, 2 * arrow,
                   width, arrow, pen);
  return composite_box (ip, bs, xs, ys);
}
...
wide_invbreve_box (path ip, SI x1, SI x2, pencil pen) {
  SI width, height;
  get_wide_parameters (x1, x2, pen, width, height);
  return arc_box (ip, 0, -height, width, (SI) (1.1 * height), 370 << 6,
                  530 << 6, pen);
}
