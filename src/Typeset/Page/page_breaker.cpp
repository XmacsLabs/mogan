...
struct page_breaker_rep {
  ...
  page_breaker_rep (array<page_item> l, space ph, int quality, space fn_sep,
                    space fnote_sep, space float_sep, font fn, int fp);
...
page_breaker_rep::page_breaker_rep (array<page_item> l2, space ph, int quality2,
                                    space fn_sep2, space fnote_sep2,
                                    space float_sep2, font fn2, int fp2)
    : l (l2), papyrus_mode (ph == as_space (as_tree ((MAX_SI >> 1)))),
      height (ph), fn_sep (fn_sep2), fnote_sep (fnote_sep2),
      float_sep (float_sep2), fn (fn2), first_page (fp2), flow_id (-1),
      brk_nr (-1), quality (quality2) {}
...
void
page_breaker_rep::init_flows (array<page_item> l, int start, int end, path p,
                              path flb) {
  int i;
  for (i= start; i < end; i++) {
    ...
    flow_tot[id] << (nr == 0 ? space (0) : flow_tot[id][nr - 1]) +
                        flow_ht[id][nr];
    flow_cont[id] << (nr == 0 ? 1 : flow_cont[id][nr - 1] + (cont ? 0 : 1));
    if ((i == end - 1) || (l[i]->nr_cols != l[i + 1]->nr_cols))
      l[i]->penalty= 0;
  }
...
page_breaker_rep::spool_break (vbreak& br, path flb, path start) {
  int id;
  for (id= 0; id < nr_flows; id++)
    if (path_up (flow_fl[id]) == flb) br[id]= fast_find (flow[id], start);
}

void
page_breaker_rep::rewind_break (vbreak& br, path flb, int& best_id,
                                path& best_p) {
  int id;
  best_id= -1;
  best_p = path ();
...
// by break points for subflows of the flow with identifier 'id'
{
  ...
  if (sid == nr_flows) {
    ...
    int           j;
    array<vbreak> brk;
    array<vbreak> ref= generate_breaks (copy (br), id, path_up (flow_fl[sid]));
    for (j= 0; j < N (ref); j++)
...
// for the flow with identifier 'id' (which may be -1 for the top flow)
{
  ...
  if (id == -1) {
    halt= path (sub_end);
    for (sid= 0; sid < nr_flows; sid++)
      if (is_atom (flow_fl[sid])) br[sid]= 0;
  }
  else if (br[id] == 0) {
    halt= path (0);
    for (sid= 0; sid < nr_flows; sid++)
      if (path_up (flow_fl[sid]) == flb) br[sid]= 0;
  }
  else {
  ...
  while (true) {
    ...
    if (best_sid == -1) break;
    br[best_sid]++;
    if (correct_break (br)) brk << generate_breaks (copy (br), best_sid);
  }
  return brk;
}
...
page_breaker_rep::compute_total_height (vbreak br) {
  ...
  for (id= 0; id < nr_flows; id++) {
    ...
    if (!is_atom (fl)) {
      ...
      if (ch == 1)
        stot+= space (cont * float_sep->min, (3 * cont * float_sep->def) / 2,
...
page_breaker_rep::init_ladders () {
  ...
  for (i= 0; i < nrb; i++) {
    ...
    new_ld[0]= i;
    for (j= 0; j < N (ld); j++)
      if (!inf_eq (brk[ld[j]], brk[i])) new_ld << ld[j];
    ld         = new_ld;
    brk_prev[i]= ld;
  }
  ...
  for (i= nrb - 1; i >= 0; i--) {
    ...
    new_ld[0]= i;
    for (j= 0; j < N (ld); j++)
      if (!inf_eq (brk[i], brk[ld[j]])) new_ld << ld[j];
    ld         = new_ld;
    brk_next[i]= ld;
  }
...
page_breaker_rep::dec_merge (ladder ld1, ladder ld2) {
  ...
  for (i= 0, j= 0; (i < N (ld1)) || (j < N (ld2));) {
    if ((j == N (ld2)) || ((i < N (ld1)) && (ld1[i] > ld2[j]))) {
      for (k= 0; k < N (ld2); k++)
        if (inf_eq (brk[ld1[i]], brk[ld2[k]])) break;
      if (k == N (ld2)) ld << ld1[i];
      i++;
      continue;
    }
    if ((i == N (ld1)) || ((j < N (ld2)) && (ld2[j] > ld1[i]))) {
      for (k= 0; k < N (ld1); k++)
        if (inf_eq (brk[ld2[j]], brk[ld1[k]])) break;
      if (k == N (ld1)) ld << ld2[j];
      j++;
      continue;
    }
...
page_breaker_rep::inc_merge (ladder ld1, ladder ld2) {
  ...
  for (i= 0, j= 0; (i < N (ld1)) || (j < N (ld2));) {
    if ((j == N (ld2)) || ((i < N (ld1)) && (ld1[i] < ld2[j]))) {
      for (k= 0; k < N (ld2); k++)
        if (inf_eq (brk[ld2[k]], brk[ld1[i]])) break;
      if (k == N (ld2)) ld << ld1[i];
      i++;
      continue;
    }
    if ((i == N (ld1)) || ((j < N (ld2)) && (ld2[j] < ld1[i]))) {
      for (k= 0; k < N (ld1); k++)
        if (inf_eq (brk[ld1[k]], brk[ld2[j]])) break;
      if (k == N (ld1)) ld << ld2[j];
      j++;
      continue;
    }
...
page_breaker_rep::correct_pagelet (int start, int end) {
  int id, mid;
  for (id= 0; id < nr_flows; id++)
    if (brk[start][id] > brk[end][id]) return false;
  for (id= 0; id < nr_flows; id++)
...
page_breaker_rep::make_insertion (int id, int ch, int i1, int i2, bool flag) {
  ...
  space spc;
  if (i1 == 0) {
    if (i2 > 1) spc= copy (flow_tot[id][i2 - 2]);
  }
...
page_breaker_rep::format_insertion (insertion& ins, double stretch) {
  ...
  for (i= 0; i < k; i++) {
    ...
    if (ht > pg->ht->max) pg_stretch= 1.0;
      ...
      pg_stretch= ((double) (ht - pg->ht->def)) /
...
page_breaker_rep::format_pagelet (pagelet& pg, space ht, bool last_page) {
  ...
  if (last_page && (pg->ht->def <= ht->def)) {
    ...
    if (ht->def > pg->ht->def) {
      // cout << "Stretch" << LF;
      stretch= ((double) (ht->def - pg->ht->def)) /
               ((double) (pg->ht->max - pg->ht->def));
    }
    ...
    stretch= -1.0;
    double factor=
        ((double) max (pg->ht->def, 1)) / ((double) max (ht->def, 1));
...
void
page_breaker_rep::search_mcol_breaks (vbreak br1, vbreak br2,
                                      array<array<int>> part, path p1, path p2,
                                      int& i1, int& i2) {
  ...
  for (id= 0; id < nr_flows; id++) {
    ...
    */
    ...
    for (i= N (part[id]) - 2; i >= 0; i--)
      if (var_path_inf_eq (p2, flow[id][part[id][i]])) br2[id]= part[id][i];
  }

  // cout << "Search breaks " << br1 << " -- " << br2 << LF;
  ASSERT (brk_nr->contains (br1) && brk_nr->contains (br2), "break not found");
  i1= brk_nr[br1];
  i2= brk_nr[br2];
}
...
page_breaker_rep::make_multi_column (skeleton sk, int real_nr_cols) {
  ...
  if (ht->max < ht->min) {
    ...
    for (i= 1; i < nr_cols; i++)
      if (sk[i - 1]->ht->min < sk[i]->ht->min) pen+= LONGER_LATTER_COLUMN;
  }
  else {
...
insertion
page_breaker_rep::make_multi_column (int start, int end, path flb,
                                     int nr_cols) {
  if ((quality > 1) && (nr_cols == 2)) return make_two_column (start, end, flb);
  ...
  for (col= 0; col < nr_cols; col++) {
    ...
    while (col_end < end) {
      if (correct_pagelet (col_start, col_end) &&
        ...
        if ((pg->pen < vpenalty (HYPH_INVALID)) && (pg->ht->def >= col_ht)) {
          if (N (pg->ins) != 0) sk << pg;
          break;
        }
      }
  ...
  while (N (tc_ld) != 0) {
    ...
    if ((status == BAD_BREAK) && (tc_bpen < vpenalty (HYPH_INVALID)))
      ...
      if (tc_middle + 1 >= tc_end) tc_ld= ladder ();
        ...
        tc_ld= inc_merge (brk_next[tc_middle + 1], sub (tc_ld, 1, N (tc_ld)));
    }
  }

  tc_ld= ladder ();
  if (((tc_ref - 1) > tc_start) && ((tc_ref - 1) < tc_end))
    tc_ld << (tc_ref - 1);
  while (N (tc_ld) != 0) {
    // cout << "Two column ladder= " << tc_ld << LF;
    tc_middle = tc_ld[0];
    ...
    if ((status == BAD_BREAK) && (tc_bpen < vpenalty (HYPH_INVALID)))
      ...
      if (tc_middle - 1 <= tc_start) tc_ld= ladder ();
        ...
        tc_ld= dec_merge (brk_prev[tc_middle - 1], sub (tc_ld, 1, N (tc_ld)));
    }
  }

  if (is_nil (tc_bpg1)) sk << make_pagelet (tc_start, tc_end, flb, 2);
  else {
    sk << tc_bpg1;
    sk << tc_bpg2;
  }
...
page_breaker_rep::fast_break_page (int i1, int& first_end) {
  ...
  while (true) {
    space spc;
    if (i1 == 0) {
      if (i2 > 1) spc= copy (flow_tot[0][i2 - 2]);
    }
...
page_breaker_rep::fast_assemble_skeleton (skeleton& sk) {
  ...
  int first_end= 0;
  for (i= 0; i < n; i++)
    if (best_prev[i] != -1) fast_break_page (i, first_end);
  ...
  while (N (cur_ld) != 0) {
    ...
    if ((status == BAD_BREAK) && (best_pen < vpenalty (HYPH_INVALID)))
      ...
      if (cur_end + 1 >= N (brk)) cur_ld= ladder ();
        ...
        cur_ld= inc_merge (brk_next[cur_end + 1], sub (cur_ld, 1, N (cur_ld)));
    }
  }
  ...
  while (N (cur_ld) != 0) {
    ...
    if ((status == BAD_BREAK) && (best_pen < vpenalty (HYPH_INVALID)))
      ...
      if (cur_end - 1 <= cur_start) cur_ld= ladder ();
        ...
        cur_ld= dec_merge (brk_prev[cur_end - 1], sub (cur_ld, 1, N (cur_ld)));
    }
  }
}
...
page_breaker_rep::assemble_skeleton (skeleton& sk) {
  ...
  if (quality > 0) {
    while (cur_start != brk_last) {
      if (best_prev[cur_start] != -1) find_next_breaks ();
      // cout << HRULE << LF << LF;
      cur_start++;
    }
...
page_breaker_rep::make_skeleton () {
  ...
  if (i < j) {
    if (dpage_flag && ((N (sk) & 1) == 1)) sk << pagelet (space (0));
    last_page_flag= true;
...
skeleton new_break_pages (array<page_item> l, space ph, int qual, space fn_sep,
                          space fnote_sep, space float_sep, font fn,
                          int first_page);

skeleton
break_pages (array<page_item> l, space ph, int qual, space fn_sep,
             space fnote_sep, space float_sep, font fn, int first_page) {
  if (get_user_preference ("new style page breaking") != "off")
    return new_break_pages (l, ph, qual, fn_sep, fnote_sep, float_sep, fn,
                            first_page);
  else {
    page_breaker_rep* H= tm_new<page_breaker_rep> (
        l, ph, qual, fn_sep, fnote_sep, float_sep, fn, first_page);
    // cout << HRULE << LF;
