...
space
as_space (tree t) {
  if (is_func (t, TMLEN, 1)) return space ((SI) (as_double (t[0]->label)));
...
new_breaker_rep::new_breaker_rep (array<page_item> l2, space ph, int quality2,
    ...
    : l (l2), papyrus_mode (ph == as_space (as_tree (MAX_SI >> 1))),
      ...
      float_sep (float_sep2), fn (fn2), first_page (fp2), quality (quality2),
      last_page_flag (true), body_ht (), body_cor (), foot_ht (), foot_tot (),
      float_ht (), float_tot (), ins_list (), best_prev (path (-1)),
      best_pens (MAX_SI), todo_list (false), done_list (false),
      cache_uniform (array<path> ()), cache_colbreaks (array<path> ()) {
  ...
  for (int i= 0; i + 1 < N (l); i++)
    if (l[i]->nr_cols == 1 && N (l[i]->fl) > 0) {
      ...
      for (int j= 0; j < N (l[i]->fl); j++) {
        ...
        if (is_tuple (lvs->channel, "footnote") && N (lvs->l) > 0 &&
            lvs->l[0]->nr_cols == 2)
          nfl << l[i]->fl[j];
        else ofl << l[i]->fl[j];
      }
  ...
  for (int i= 0; i < N (l); i++) {
    ...
    if ((i == N (l) - 1) || (l[i]->nr_cols != l[i + 1]->nr_cols))
      l[i]->penalty= 0;
    ...
    for (int j= 0; j < k; j++) {
      ...
      if (ins->nr_cols == 1 && l[i]->nr_cols > 1) {
        if (is_tuple (lvs->channel, "footnote")) wide_spc+= ins->ht + fn_sep;
        ...
        if (is_tuple (lvs->channel, "footnote")) foot_spc+= ins->ht + fn_sep;
        else if (is_tuple (lvs->channel, "float")) {
...
new_breaker_rep::compute_space (path b1, path b2, bool wide_part) {
  // cout << "    Compute space " << b1 << ", " << b2 << ", " << wide_part <<
  // LF;
  ...
  space spc;
  if (i1 == 0) {
    if (i2 > 1) spc= copy (body_tot[i2 - 2]);
  }
  ...
  if (!wide_part &&
      foot_tot[i2 - 1]->def > (i1 == 0 ? 0 : foot_tot[i1 - 1]->def)) {
...
new_breaker_rep::last_break (path b) {
  return last_page_flag &&
         (b == path (N (l)) || (must_new[b->item] && is_nil (b->next)));
}
...
new_breaker_rep::find_page_breaks (path b1) {
  ...
  path     b2= b1;
  while (true) {
    if (height->def >= (1 << 28) && b2->item < n) b2= path (n);
    else if (!is_nil (b2->next)) b2= path (b2->item, b2->next->next->next);
    else if (b2->item >= n) break;
    else {
      int i= b2->item;
      if (N (ins_list[i]) != 0 &&
    ...
    if (bpen < HYPH_INVALID) {
      ...
      if (!last_break (b2)) pen+= as_vpenalty (spc->def - height->def);
...
new_breaker_rep::make_insertion (int i1, int i2) {
  ...
  space     spc;
  if (i1 == 0) {
    if (i2 >= 2) spc= copy (body_tot[i2 - 2]);
  }
  else {
    spc= body_tot[i2 - 2] - body_tot[i1 - 1];
  }
  SI top_cor= body_cor[i1]->max;
  SI bot_cor= body_cor[i2 - 1]->min;
  spc+= space (top_cor + body_cor[i2 - 1]->def + bot_cor);
...
new_breaker_rep::assemble_skeleton (skeleton& sk, path end, int& offset) {
  ...
  if (must_break[begin->item] && begin->item < end->item) {
    if (l[begin->item]->t == NEW_DPAGE)
      if (((N (sk) + offset) & 1) == 1) sk << pagelet (space (0));
...
skeleton
new_break_pages (array<page_item> l, space ph, int qual, space fn_sep,
                 space fnote_sep, space float_sep, font fn, int first_page) {
  new_breaker_rep* H= tm_new<new_breaker_rep> (l, ph, qual, fn_sep, fnote_sep,
                                               float_sep, fn, first_page);
  // cout << HRULE << LF;
  H->find_page_breaks ();
  // cout << HRULE << LF;
  skeleton sk;
