...
struct lb_info {
  ...
  operator tree () {
    return tuple (as_tree (rep->prev), as_string (rep->pen),
...
operator<< (tm_ostream& out, lb_info hi) {
  return out << "[ " << hi->prev << ", " << hi->pen << ", " << hi->pen_spc
             << " ]";
}
...
struct line_breaker_rep {
  ...
  line_breaker_rep (array<line_item> a, int start, int end, SI line_width,
...
line_breaker_rep::line_breaker_rep (array<line_item> a2, int start2, int end2,
                                    SI line_width2, SI large_width2,
                                    SI first_spc2, SI last_spc2)
    : a (a2), start (start2), end (end2), line_width (line_width2),
      large_width (large_width2), first_spc (first_spc2), last_spc (last_spc2),
      best (lb_info ()) {}
...
hyphenate (line_item item, int pos, line_item& item1, line_item& item2) {
  ...
  item1= line_item (STRING_ITEM, OP_SKIP,
                    ...
                    hp[pos], item->lan);
  item2= line_item (STRING_ITEM, item->op_type, text_box (ip, x2, s2, fn, pen),
...
line_breaker_rep::next_ragged_break (path pos) {
  ...
  while (true) {
    if (cur_nr < end) {
      cur_spc-= cur_item->b->w ();
      if ((cur_spc <= line_width) && (cur_item->type == STRING_ITEM)) {
        string s= cur_item->b->get_leaf_string ();
        if (N (s) > 4) {
          array<int> hp= cur_item->lan->get_hyphens (s);
          int        i = get_position (cur_item->b->get_leaf_font (), s,
                                       line_width - cur_spc);
...
void
line_breaker_rep::test_better (path new_pos, path old_pos, int pen,
                               PEN pen_spc) {
  ...
  if ((pen < cur->pen) || ((pen == cur->pen) && (pen_spc < cur->pen_spc))) {
    cur->prev   = old_pos;
    cur->pen    = pen;
...
bool
line_breaker_rep::propose_break (path new_pos, path old_pos, int pen,
                                 space spc) {
...
line_breaker_rep::process (path pos) {
  ...
  if ((pass > 1) || (best[pos]->pen < HYPH_INVALID)) {
    ...
    for (i= pos->item; i < end; i++) {
      ...
      if ((spc->max > line_width) && (item->type == STRING_ITEM) &&
          (N (item->b->get_leaf_string ()) > 4))
        break_string (item, pos, i, spc + space (-item->b->w ()));
      if (item->penalty < HYPH_INVALID)
        if (propose_break (path (i + 1), pos, item->penalty, spc)) break;
      if ((item->type == CONTROL_ITEM) && (item->t == LINE_BREAK) &&
          (spc->min < line_width))
        if (propose_break (path (i + 1), pos, 0, space (line_width))) break;
    }
    if (i == end) {
      line_width-= last_spc;
  ...
  if (first->type == STRING_ITEM) {
    ...
    if (n > 4)
      for (i= 0; i < n - 1; i++)
        if (best->contains (pos * i)) process (pos * i);
  }
}
...
line_breaker_rep::compute_breaks () {
  ...
  for (i= ap[N (ap) - 2]->item; i < end; i++)
    if (a[i]->b->w () + a[i]->spc->def != 0) return ap;
  ap[N (ap) - 2]= ap[N (ap) - 1];
  ap->resize (N (ap) - 1);
  return ap;
}
...
array<path>
line_breaks (array<line_item> a, int start, int end, SI line_width,
             SI large_width, SI first_spc, SI last_spc, bool ragged) {
  ...
  line_breaker_rep* H= tm_new<line_breaker_rep> (
      a, start, end, line_width, large_width, first_spc, last_spc);
