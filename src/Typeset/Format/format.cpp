...
format_cell_rep::equal (format fm) {
  ...
  return (fw->width == width) && (fw->vpos == vpos) && (fw->depth == depth) &&
         (fw->height == height);
}
...
format_vstream_rep::equal (format fm) {
  ...
  return (fw->width == width) && (fw->before == before) && (fw->after == after);
}

format_vstream_rep::operator tree () {
  return tuple ("format vstream", as_string (width), as_string (N (before)),
                as_string (N (after)));
}
...
query_vstream_width_rep::operator tree () {
  return tuple ("query vstream width", as_string (N (before)),
                as_string (N (after)));
}
