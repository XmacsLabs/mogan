...
bool
bridge_compound_rep::notify_macro (int type, string var, int l, path p,
                                   tree u) {
  /*
      ...
      for (i= 0; i < n; i++)
        if (is_atomic (fun[i])) {
          ...
          env->macro_src->item (var)=
              i + delta < m ? descend (ip, i + delta) : decorate_right (ip);
        }
...
bridge_compound_rep::my_typeset (int desired_status) {
  ...
  if (is_applicable (f)) {
    ...
    if (!the_drd->is_child_enforcing (st)) ttt->insert_marker (st, ip);
