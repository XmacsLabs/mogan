...
resize (edit_env env, SI old, SI minimum, SI maximum, tree new_size) {
  ...
  if (flag) {
    ...
    case '-':
      return offset - arg;
    case '[':
      return min (offset, arg);
    case ']':
      return max (offset, arg);
    default:
      return arg;
    }
  }
...
void
get_canvas_horizontal (canvas_properties props, SI bx1, SI bx2, SI& x1, SI& x2,
                       SI& scx) {
  ...
  if (type != "plain") {
    if (ends (type, "w") || ends (type, "e")) x2= max (x1, x2 - (bwid + bpad));
    x1+= (border + props->hpadding);
    x2-= (border + props->hpadding);
  }
  if (is_atomic (props->scx) && ends (props->scx->label, "%")) {
    double p=
        as_double (props->scx->label (0, N (props->scx->label) - 1)) / 100.0;
    SI d = ((x2 - x1) - (bx2 - bx1));
    SI dx= (d >= 0 ? 0 : (SI) (p * d));
    scx  = dx + x1 - bx1;
  }
...
void
get_canvas_vertical (canvas_properties props, SI by1, SI by2, SI& y1, SI& y2,
                     SI& scy) {
  edit_env env   = props->env;
  string   type  = props->type;
  SI       bwid  = props->bar_width;
  ...
  if (is_atomic (props->scy) && ends (props->scy->label, "%")) {
    double p=
        as_double (props->scy->label (0, N (props->scy->label) - 1)) / 100.0;
    SI d = ((y2 - y1) - (by2 - by1));
    SI dy= (d >= 0 ? d : (SI) (p * d));
    scy  = dy + y1 - by1;
  }
...
box
put_scroll_bars (canvas_properties props, box b, path ip, box inner, SI scx,
                 SI scy) {
  string type  = props->type;
  ...
  array<SI>  xs (1);
  array<SI>  ys (1);
  b    = resize_box (dip, b, b->x1 - hpad, b->y1 - vpad, b->x2 + hpad,
                     b->y2 + vpad);
