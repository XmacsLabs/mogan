...
edit_math_rep::make_script (bool sup, bool right) {
  ...
  if (selection_active_small ()) insert_tree (tree (s, selection_get_cut ()));
  else {
    path p= path_up (tp);
    tree t= subtree (et, p);
    bool flag;

    if (is_format (as_tree (p))) TM_FAILED ("bad cursor position");
    if (t == "" && is_script (subtree (et, path_up (p)))) return;
...
edit_math_rep::make_var_sqrt () {
  if (selection_active_small ()) {
    ...
    if (is_func (t, SQRT, 1)) insert_tree (tree (SQRT, t[0], ""), path (1, 0));
    else insert_tree (tree (SQRT, t, ""), path (1, 0));
  }
  else {
    insert_tree (tree (SQRT, "", ""), path (0, 0));
    set_message (
        concat (kbd ("left"), ": set n", kbd ("right"), ": when finished"),
        "n-th root");
  }
}
...
edit_math_rep::make_neg () {
  if (selection_active_small ()) insert_tree (tree (NEG, selection_get_cut ()));
  else {
...
edit_math_rep::back_around (tree t, path p, bool forward) {
  ...
  if (is_func (t, BIG_AROUND)) {
    if (match || forward) go_to_border (p * 1, forward);
    else {
      remove_node (t, 1);
      correct (path_up (p));
    }
    ...
    if (is_deleted (t[i]))
      ;
    else if (is_atomic (t[i])) assign (t[i], "<nobracket>");
    else if (is_func (t[i], LEFT)) assign (t[i], tree (LEFT, "."));
    else if (is_func (t[i], RIGHT)) assign (t[i], tree (RIGHT, "."));
...
edit_math_rep::back_in_around (tree t, path p, bool forward) {
  ...
  if (is_empty (t[1]) && match) {
    ...
    if (match || forward) go_to_border (path_up (p), !forward);
    else {
      remove_node (t, 1);
      correct (path_up (p, 2));
    }
...
edit_math_rep::back_in_long_arrow (tree t, path p, bool forward) {
  ...
  if (i == 2) {
    ...
    else if (forward && N (t) >= 3) go_to_border (path_up (p) * 2, forward);
...
edit_math_rep::pre_remove_around (path p) {
  ...
  if (is_script (st[1]) || is_prime (st[1])) assign (st[1], "");
    ...
    while (li < N (st[1])) {
      tree sst= st[1][li];
      if (!is_func (sst, RSUB) && !is_func (sst, RSUP) &&
          !is_func (sst, RPRIME))
        break;
      li++;
    }
    while (ri >= 0) {
      tree sst= st[1][ri];
      if (!is_func (sst, LSUB) && !is_func (sst, LSUP) &&
          !is_func (sst, LPRIME))
        break;
      ri--;
    }
