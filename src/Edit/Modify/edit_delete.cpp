...
void
edit_text_rep::get_deletion_point (path& p, int& last, int& rix, tree& t,
                                   tree& u, bool forward) {
...
edit_text_rep::remove_text_sub (bool forward) {
          ...
          if (t == tree (DOCUMENT, "")) {
            ...
            if (is_func (subtree (et, path_up (p, 2)), INACTIVE))
              remove_structure (forward);
                 ...
                 is_func (u, TFORMAT)) {
          if (t == tree (DOCUMENT, "")) back_in_table (u, p, forward);
        }
        else if (is_func (u, DOCUMENT_AT)) back_in_text_at (u, p, forward);
      }
      return;
    }
    else {
  ...
  if (last == (forward ? 0 : 1)) switch (L (t)) {
    case RAW_DATA:
    case HSPACE:
    case VAR_VSPACE:
    case VSPACE:
  ...
  if (last == (forward ? rix : 0)) {
    ...
    case LOCUS:
      ...
      return;
    default:
      if (is_graphical_text (u)) back_in_text_at (u, p, forward);
      else if (is_compound (u, "cell-inert") || is_compound (u, "cell-input") ||
...
edit_text_rep::remove_structure_upwards () {
  path p= path_up (tp);
  while ((rp < p) && is_format (subtree (et, path_up (p))))
    p= path_up (p);
  if (!(rp < p)) return;
  int last= last_item (p);
  p       = path_up (p);
  tree st = subtree (et, p);
  if (is_func (st, AROUND, 3) || is_func (st, VAR_AROUND, 3) ||
      is_func (st, BIG_AROUND, 2))
    pre_remove_around (p);
  bool recurse= is_func (st, TFORMAT) || is_func (st, TABLE) ||
                is_func (st, ROW) || is_func (st, CELL) ||
                is_compound (st, "shown") || drd->var_without_border (L (st));
