...
drd_correct (drd_info drd, tree t) {
  if (is_atomic (t)) return t;
    ...
    int i, n= N (t);
    if (drd->contains (as_string (L (t))) && !drd->correct_arity (L (t), n))
      return "";
    tree r (t, n);
    for (i= 0; i < n; i++)
...
is_block_pseudo_code (tree t) {
  ...
  return s == "algo-procedure" || s == "algo-function" || s == "algo-for" ||
         s == "algo-forall" || s == "algo-foreach" || s == "algo-while" ||
         s == "algo-repeat" || s == "algo-loop" || s == "algo-body" ||
         s == "algo-begin" || s == "algo-inputs" || s == "algo-outputs" ||
         s == "algo-if" || s == "algo-else-if" || s == "algo-else";
}
...
is_migratable (tree t) {
  return t == " " || is_compound (t, "mlx", 2) || is_func (t, VAR_VSPACE) ||
         is_func (t, VSPACE) ||
  ...
  if (is_concat (t)) {
    ...
    for (i= 0; i < n;)
      if (is_multi_paragraph (t[i])) {
        int bef_i= i;
        while (bef_i > start && is_migratable (t[bef_i - 1]))
          bef_i--;
        int aft_i= i + 1;
        while (aft_i < n && is_migratable (t[aft_i]))
          aft_i++;
...
superfluous_with_correct (tree t, tree env) {
  if (is_atomic (t)) return t;
    ...
    if (is_func (t, WITH) && ((N (t) & 1) == 0)) t= t * tree (WITH, "");
    tree r (t, N (t));
    for (int i= 0; i < N (t); i++)
      r[i]= superfluous_with_correct (t[i], the_drd->get_env_child (t, i, env));
    ...
    if (a[i] == "<minus>") r << tree ("-");
      ...
      if (j1 < 0 || j2 >= N (a))
        ;
               ...
               ((tp[j1] == SYMBOL_BASIC) || (tp[j1] == SYMBOL_POSTFIX)) &&
               ((tp[j2] == SYMBOL_BASIC) || (tp[j2] == SYMBOL_PREFIX)))
        r << tree ("<setminus>");
      else r << a[i];
    }
...
homoglyph_correct (tree t, string mode) {
  ...
  if (is_compound (t)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      if (is_correctable_child (t, i)) r[i]= homoglyph_correct (t[i], smode);
      else r[i]= t[i];
    }
  }
  ...
  for (int i= 0; i < N (a); i++)
    if (a[i] == " " || a[i] == "*") {
      ...
      if (j1 < 0 || j2 >= N (a))
        ...
        ;
      else if (tp[j1] == SYMBOL_PREFIX || tp[j1] == SYMBOL_INFIX ||
               tp[j1] == SYMBOL_PREFIX_INFIX || tp[j1] == SYMBOL_SEPARATOR ||
               tp[j1] == SYMBOL_PROBABLE_MIDDLE)
        ;
      else if (tp[j2] == SYMBOL_POSTFIX || tp[j2] == SYMBOL_INFIX ||
               tp[j2] == SYMBOL_PREFIX_INFIX || tp[j2] == SYMBOL_SEPARATOR ||
...
superfluous_invisible_correct (tree t, string mode) {
  ...
  if (is_compound (t)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      //<< ", " << smode << "\n";
      if (is_func (t, WITH) && i != N (t) - 1) r[i]= t[i];
  ...
  if (is_func (r, INACTIVE, 1) && is_func (r[0], RIGID)) return r[0];
  else if (mode == "math") {
    array<tree> a= concat_tokenize (r);
...
invisible_corrector::contains_infix (tree t) {
  ...
  for (int i= 0; i < N (tp); i++)
    if (tp[i] == SYMBOL_INFIX || tp[i] == SYMBOL_PREFIX_INFIX) return true;
  return false;
}
...
invisible_corrector::count_invisible (array<tree> a) {
  ...
  for (int i= 0; i < N (a); i++)
    if (is_atomic (a[i]) && is_letter_like (a[i]->label)) {
      ...
      string s= a[i]->label;
      if (j1 >= 0) {
        if (a[j1] == "*") times_before (s)= times_before[s] + 1;
        if (a[j1] == " ") space_before (s)= space_before[s] + 1;
      }
      if (j2 < N (a)) {
        if (a[j2] == "*") times_after (s)= times_after[s] + 1;
        if (a[j2] == " ") space_after (s)= space_after[s] + 1;
...
invisible_corrector::count_invisible (tree t, string mode) {
  if (is_compound (t)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      if (is_func (t, WITH) && i != N (t) - 1)
        ;
      else if (is_correctable_child (t, i)) count_invisible (t[i], smode);
    }
  }
  if (mode == "math") count_invisible (concat_tokenize (t));
}
...
invisible_corrector::get_status (tree t, bool left, bool script_flag) {
  if (is_atomic (t)) {
    ...
    if (is_numeric (s)) return (left ? SURE_TIMES : PROBABLE_TIMES);
    else if (starts (g, "Unary-operator-textual"))
      return (left ? SURE_SPACE : BOTH_WAYS);
    else if (starts (g, "Binary-operator")) return SURE_SPACE;
    else if (starts (g, "N-ary-operator"))
      return (left ? SURE_SPACE : BOTH_WAYS);
    else if (is_letter_like (s)) {
      if (left) {
        if (times_after[s] > 0 && space_after[s] == 0) return SURE_TIMES;
        else if (space_after[s] > 0 && times_after[s] == 0) return SURE_SPACE;
        else if (times_after[s] > space_after[s]) return PROBABLE_TIMES;
        else if (space_after[s] > times_after[s]) return PROBABLE_SPACE;
        else if (N (s) > 1 && is_iso_alpha (s)) return PROBABLE_SPACE;
        else if (script_flag) return PROBABLE_TIMES;
        else return BOTH_WAYS;
      }
      else {
        if (times_before[s] > space_before[s]) return PROBABLE_TIMES;
    ...
    else if (s == "<cdots>" || s == "<ldots>") return PROBABLE_TIMES;
    else return ((force > 0) ? BOTH_WAYS : SURE_NOTHING);
  }
  else {
    if (is_around (t)) {
      if (!left && contains_separator (t[1])) return PROBABLE_NOTHING;
      ...
      else if (!contains_infix (t[1])) return (left ? BOTH_WAYS : SURE_SPACE);
      else return BOTH_WAYS;
    }
    else if (is_func (t, FRAC) || is_func (t, SQRT))
      return (left ? SURE_TIMES : BOTH_WAYS);
    else if (!left && is_func (t, BIG_AROUND, 2) &&
             (t[0] == "<sum>" || t[0] == "<amalg>" || t[0] == "<oplus>" ||
              t[0] == "<uplus>" || t[0] == "<int>" || t[0] == "<oint>" ||
              t[0] == "<intlim>" || t[0] == "<ointlim>" || t[0] == "<prod>" ||
              t[0] == "<odot>" || t[0] == "<otimes>"))
      return PROBABLE_TIMES;
    else if (is_func (t, WIDE, 2)) return get_status (t[0], left, script_flag);
...
invisible_corrector::correct (array<tree> a) {
  ...
  for (int i= 0; i < N (a); i++) {
    ...
    if (a[i] != " " && tp[i] == SYMBOL_BASIC) {
      ...
      if (j >= N (a) || a[j] == " " || tp[j] != SYMBOL_BASIC) continue;
      ...
      // cout << "Pair (" << a[i] << ", " << a[j] << ")"
      //<< " -> (" << sti << ", " << stj << ")" << LF;
      if (sti == SURE_NOTHING || stj == SURE_NOTHING) ins= "";
      else if (sti == SURE_TIMES && stj != SURE_SPACE) ins= "*";
      else if (sti == SURE_SPACE && stj != SURE_TIMES) ins= " ";
      else if (sti == PROBABLE_NOTHING || stj == PROBABLE_NOTHING) ins= "";
      else if (sti == PROBABLE_TIMES && stj == PROBABLE_TIMES) ins= "*";
      else if (sti == PROBABLE_SPACE && stj == PROBABLE_SPACE) ins= " ";
      else if (sti == PROBABLE_TIMES && stj == BOTH_WAYS) ins= "*";
      else if (sti == PROBABLE_SPACE && stj == BOTH_WAYS) ins= " ";
      else if (sti == BOTH_WAYS && stj == PROBABLE_TIMES) ins= "*";
      else if (sti == BOTH_WAYS && stj == PROBABLE_SPACE) ins= " ";
...
invisible_corrector::correct (tree t, string mode) {
  ...
  if (is_compound (t)) {
    ...
    for (i= 0; i < n; i++) {
      string smode= get_submode (t, i, mode);
      if (is_func (t, WITH) && i != N (t) - 1) r[i]= t[i];
      else if (is_correctable_child (t, i)) r[i]= correct (t[i], smode);
      else r[i]= t[i];
    }
  }
...
misc_math_correct (tree t) {
  if (is_atomic (t)) return t;
    ...
    if (N (t[0][0]->label) != 1) return tree (L (t), t[0][0]);
      ...
      return tree (L (t), tree (WITH, "math-font-family", "trm",
    ...
    if (is_atomic (last) && N (last->label) > 0 &&
      ...
      string s= last->label;
      int    i= N (s);
      while (i > 0 && is_punctuation (s[i - 1]))
        i--;
  ...
  if (is_atomic (t)) return 0;
    ...
    for (int i= 0; i < N (t); i++) {
      ...
      if (cmode != "math") sum+= count_math_errors (t[i], mode);
        ...
        tree u= t[i];
        while (is_func (u, DOCUMENT, 1) || is_func (u, TFORMAT) ||
               is_func (u, WITH))
          u= u[N (u) - 1];
...
math_status_print () {
  ...
  cout << "Superfluous with corrected      : " << corrected_superfluous_with
       << "\n";
  cout << "Upgraded brackets               : " << corrected_brackets << "\n";
  cout << "Moved brackets                  : " << corrected_move_brackets
       << "\n";
  ...
  cout << "Missing invisible corrected     : " << corrected_missing_invisible
       << "\n";
  cout << "Zealous invisible corrected     : " << corrected_zealous_invisible
       << "\n";
  cout << "\n";
}
...
automatic_correct (tree t, string version) {
  if (version_inf_eq (version, "1.0.7.9")) {
    ...
    if (enabled_preference ("remove superfluous invisible"))
      t= superfluous_invisible_correct (t);
    if (enabled_preference ("homoglyph correct")) t= homoglyph_correct (t);
    if (enabled_preference ("remove superfluous invisible"))
      t= superfluous_invisible_correct (t);
    if (enabled_preference ("insert missing invisible"))
      t= missing_invisible_correct_twice (t);
...
manual_correct (tree t) {
  ...
  if (enabled_preference ("manual remove superfluous invisible"))
    t= superfluous_invisible_correct (t);
  if (enabled_preference ("manual homoglyph correct")) t= homoglyph_correct (t);
  if (enabled_preference ("manual remove superfluous invisible"))
    t= superfluous_invisible_correct (t);
