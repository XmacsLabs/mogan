...
is_dubious_open_middle (array<int> tp, int j) {
  ...
  while (j >= 0 && (tp[j] == SYMBOL_SKIP || tp[j] == SYMBOL_SCRIPT))
    j--;
  return j < 0 || tp[j] == SYMBOL_PREFIX || tp[j] == SYMBOL_INFIX ||
         tp[j] == SYMBOL_PREFIX_INFIX || tp[j] == SYMBOL_SEPARATOR;
}
...
is_dubious_close_middle (array<int> tp, int j) {
  ...
  while (j < N (tp) && (tp[j] == SYMBOL_SKIP || tp[j] == SYMBOL_SCRIPT))
    j++;
  return j >= N (tp) || tp[j] == SYMBOL_POSTFIX || tp[j] == SYMBOL_INFIX ||
         tp[j] == SYMBOL_PREFIX_INFIX || tp[j] == SYMBOL_SEPARATOR;
}
...
upgrade_probable (array<int> tp_in) {
  ...
  for (int i= 0; i < N (tp); i++)
    if (tp[i] >= SYMBOL_PROBABLE_OPEN) {
      ...
      while (j >= 0 && (tp[j] == SYMBOL_SKIP || tp[j] == SYMBOL_SCRIPT))
        j--;
      if (j < 0 || tp[j] == SYMBOL_PREFIX || tp[j] == SYMBOL_INFIX ||
          tp[j] == SYMBOL_PREFIX_INFIX || tp[j] == SYMBOL_SEPARATOR)
        tp[i]= SYMBOL_PROBABLE_OPEN;
      j= i + 1;
      while (j < N (tp) && (tp[j] == SYMBOL_SKIP || tp[j] == SYMBOL_SCRIPT))
        j++;
      if (j >= N (tp) || tp[j] == SYMBOL_POSTFIX || tp[j] == SYMBOL_INFIX ||
    ...
    if (tp[i] == SYMBOL_SEPARATOR) last_open= -1;
      ...
      if (tp[i] == SYMBOL_PROBABLE_OPEN ||
        ...
        last_open= i;
               ...
               (last_open != -1 && tp[i] == SYMBOL_PROBABLE_MIDDLE)) {
        if (last_open != -1 && a[i] == a[last_open] &&
            ((tp[last_open] == SYMBOL_PROBABLE_OPEN) ||
             (tp[i] == SYMBOL_PROBABLE_CLOSE) ||
             //(i == last_open + 2 &&
             // a[i-1] == "<cdot>") ||
             (insist && !is_dubious_open_middle (tp, last_open) &&
...
detect_probable (array<tree> a, array<int> tp_in) {
  ...
  for (int i= 0; i < N (tp); i++)
    if (tp[i] >= SYMBOL_OPEN) {
      if (tp[i] == SYMBOL_OPEN || tp[i] == SYMBOL_PROBABLE_OPEN) last_open= i;
      else if (tp[i] == SYMBOL_CLOSE || tp[i] == SYMBOL_PROBABLE_CLOSE) {
        if (last_open != -1) {
          tp[last_open]= SYMBOL_OPEN;
          tp[i]        = SYMBOL_CLOSE;
...
make_small (tree br) {
  ...
  if (is_func (br, LEFT) || is_func (br, MID) || is_func (br, RIGHT) ||
      is_func (br, BIG))
...
infix_split (array<tree> a, array<int> tp_in, array<int> pri, int level) {
  ...
  for (int i= 0; i < N (tp); i++)
    if (tp[i] == SYMBOL_OPEN_BIG) weakest= min (weakest, pri[i]);
    else if (tp[i] == SYMBOL_INFIX || tp[i] == SYMBOL_PREFIX_INFIX ||
             tp[i] == SYMBOL_SEPARATOR || tp[i] == SYMBOL_MIDDLE ||
...
postfix_split (array<tree> a, array<int> tp_in, int level) {
  ...
  int        i = N (a);
  while (i > 0 &&
         (tp[i - 1] == SYMBOL_PREFIX || tp[i - 1] == SYMBOL_INFIX ||
          tp[i - 1] == SYMBOL_PREFIX_INFIX || tp[i - 1] == SYMBOL_SEPARATOR ||
          tp[i - 1] == SYMBOL_OPEN || tp[i - 1] == SYMBOL_MIDDLE ||
          tp[i - 1] == SYMBOL_PROBABLE_OPEN ||
          tp[i - 1] == SYMBOL_PROBABLE_MIDDLE || tp[i - 1] == SYMBOL_SKIP ||
          a[i - 1] == "."))
    i--;
  if (i != N (a)) {
    array<tree> r= upgrade_brackets (range (a, 0, i), level);
...
is_concat_big (tree t) {
  ...
  for (int i= 1; i < N (t); i++)
    if (!is_func (t[i], RSUB, 1) && !is_func (t[i], RSUP, 1)) return false;
  return true;
}
...
upgrade_above_below (tree t) {
  if (is_atomic (t)) return t;
    ...
    if (is_func (r, ABOVE, 2)) {
      if (is_func (r[0], BIG)) r= tree (CONCAT, r[0], tree (RSUP, r[1]));
      else if (is_concat_big (r[0]))
        r= tree (r[0] * tree (CONCAT, tree (RSUP, r[1])));
    }
    if (is_func (r, BELOW, 2)) {
      if (is_func (r[0], BIG)) r= tree (CONCAT, r[0], tree (RSUB, r[1]));
      else if (is_concat_big (r[0]))
