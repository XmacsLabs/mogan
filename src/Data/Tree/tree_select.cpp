...
right_most_inside (path p, tree t) {
  ...
  if (N (t) == 1 && p->item == 0) return right_most_inside (p->next, t[0]);
  if (is_func (t, WIDE, 2))
...
adjust_right_script (tree t, path& o1) {
  while (is_concat (t) && o1->item > 0 && o1->next == path (0)) {
    ...
    if (is_right_script_prime (st)) {
      tree pt= t[o1->item - 1];
      if (!is_atomic (pt)) o1= path (o1->item - 1, start (pt));
      else {
        string s  = pt->label;
        int    pos= N (s);
        while (pos > 0) {
...
adjust_right_script (tree t, path& o1, path& o2) {
  ...
  if (is_right_script_prime (t[o2->item])) {
    int i= o2->item;
    while (i + 1 < N (t) && is_right_script_prime (t[i + 1]))
      i++;
    o2= path (i, 1);
  }
}
...
adjust_left_script (tree t, path& o1, path& o2) {
  ...
  if (is_left_script_prime (t[o1->item])) {
    int i= o1->item;
    while (i >= 1 && is_left_script_prime (t[i - 1]))
      i--;
    o1= path (i, 0);
  }
}
...
selection_make_accessible (tree t, path i1, path i2, path& o1, path& o2) {
  o1= i1;
  o2= i2;
  if (!is_accessible_cursor (t, o1)) o1= previous_accessible (t, o1);
  if (!is_accessible_cursor (t, o1)) o1= next_accessible (t, o1);
  if (!is_accessible_cursor (t, o2)) o2= next_accessible (t, o2);
  if (!is_accessible_cursor (t, o2)) o2= previous_accessible (t, o2);
  if (path_inf (o1, o2)) o1= shift (t, o1, 1);
}
...
table_search_format (tree t, path p) {
  ...
  while ((!is_nil (p)) && (!is_func (subtree (t, p), TABLE)))
    p= path_up (p);
...
is_table_selection (tree et, path p1, path p2, bool strict) {
  ...
  tree t= subtree (et, p);
  return is_func (t, TFORMAT) || is_func (t, TABLE) || is_func (t, ROW) ||
         is_func (t, CELL) ||
         (!strict && is_compound (t) && N (t) == 1 && is_func (t[0], TFORMAT));
}

path
find_subtable_selection (tree et, path p1, path p2, int& row1, int& col1,
                         int& row2, int& col2) {
