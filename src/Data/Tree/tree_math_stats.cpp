...
strip_decorations (tree t) {
  ...
  else if (is_func (t, NEG)) return strip_decorations (t[0]);
  else if (is_atomic (t) && is_numeric (t->label)) return "1";
  else if (is_func (t, SQRT)) return tree (SQRT, "1");
  else if (is_func (t, FRAC)) return tree (FRAC, "1", "1");
  else return t;
}
...
root_index (array<tree> a, int i) {
  int j= i;
  while (j > 0 && (is_func (a[j], RSUB) || is_func (a[j], RSUP) ||
                   is_func (a[j], RPRIME)))
    j--;
  return j;
}
...
math_stats::compile (array<tree> a, tree parent) {
  ...
  for (int i= 0; i < N (a); i++) {
    ...
    if ((i + 1) < N (a) &&
      ...
      if (N (v) >= 1) {
        c        = tree (CONCAT, l,
                         tree (AROUND, b[0], strip_decorations (v[0]), b[2]));
        roles (c)= roles[c] + 1;
      }
  ...
  if (is_compound (t)) {
    ...
    for (i= 0; i < n; i++) {
      ...
      if (is_func (t, WITH) && i != N (t) - 1)
        ...
        ;
      else if (is_correctable_child (t, i)) compile (t[i], t, smode);
    }
  }
  if (mode == "math") {
