...
void
spell_string (tree lan, range_set& sel, string s, path p, int pos1, int pos2) {
  int pos= pos1;
...
spell (tree mode, tree lan, range_set& sel, tree t, path p) {
  ...
  if (is_atomic (t)) {
    if (mode == "text") spell_string (lan, sel, t->label, p, 0, N (t->label));
  }
  else
    for (int i= 0; i < N (t); i++)
...
void
spell (tree mode, tree lan, range_set& sel, tree t, path p, path pos1,
       path pos2) {
  if (is_nil (pos1) || is_nil (pos2))
    ;
  else if (is_atomic (t))
    spell_string (lan, sel, t->label, p, max (pos1->item, 0),
                  min (pos2->item, N (t->label)));
  else if (pos1 == path (0) && pos2 == path (1)) spell (mode, lan, sel, t, p);
  else if (!is_atom (pos1) && !is_atom (pos2) && pos1->item == pos2->item)
    spell (mode, lan, sel, t[pos1->item], p * pos1->item, pos1->next,
           pos2->next);
  else {
    int i1= 0, i2= N (t);
    ...
    for (int i= i1; i < i2; i++) {
      ...
      if (is_accessible_for_spell (t, i)) {
        ...
        if (i == i1 && !is_atom (pos1) && i1 + 1 < i2)
          spell (smode, slan, sub[i], t[i], p * i, pos1->next,
                 path (right_index (t[i])));
        else if (i == i2 - 1 && !is_atom (pos2) && i1 + 1 < i2)
          spell (smode, slan, sub[i], t[i], p * i, path (0), pos2->next);
        else spell (smode, slan, sub[i], t[i], p * i);
        hits+= N (sub[i]);
      }
    }
    for (int i= i1; i < i2; i++)
