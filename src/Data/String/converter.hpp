...
public:
  inline converter_rep (string from2, string to2)
      : rep<converter> (from2 * "-" * to2), ht (), output (), nil_string (),
        from (from2), to (to2), copy_unmatched (true) {
    load ();
  }
...
void hashtree_from_dictionary (hashtree<char, string> dic, string file_name,
                               escape_type key_escape= BIT2BIT,
                               escape_type val_escape= UTF8,
                               bool        reverse   = false);
