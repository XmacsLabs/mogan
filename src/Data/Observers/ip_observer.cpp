...
ip_observer_rep::announce (tree& ref, modification mod) {
  ...
  parent->data->announce (parent, ip->item * mod);
}
...
ip_observer_rep::done (tree& ref, modification mod) {
  ...
  if (!has_parent (ip)) return;
  tree& parent (subtree (the_et, reverse (ip->next)));
  parent->data->done (parent, ip->item * mod);
}
...
ip_observer_rep::touched (tree& ref, path p) {
  ...
  if (!has_parent (ip)) return;
  tree& parent (subtree (the_et, reverse (ip->next)));
  parent->data->touched (parent, path (ip->item, p));
}
...
void
ip_observer_rep::notify_assign_node (tree& ref, int op) {
...
ip_observer_rep::set_ip (path ip2) {
  if (is_nil (ip) || is_nil (ip2)) TM_FAILED ("cannot alter global root");
  ip->item= ip2->item;
  ip->next= ip2->next;
  return true;
}
...
attach_ip (tree& ref, path ip) {
  ...
  if (is_nil (ref->data) || !ref->data->set_ip (ip)) {
    // cout << "Create ip observer " << ip << " for " << ref << "\n";
    ref->data= list_observer (ip_observer (ip), ref->data);
  }
  if (is_compound (ref)) {
    int i, n= N (ref);
    for (i= 0; i < n; i++) {
...
detach_ip (tree& ref) {
  ...
  if (!is_nil (ref->data)) (void) ref->data->set_ip (DETACHED);
}

path
obtain_ip (tree& ref) {
  path ip;
  if (is_nil (ref->data)) return DETACHED;
  if (!ref->data->get_ip (ip)) return DETACHED;
  path p= ip;
  if (!is_nil (p)) {
