...
register_link (soft_link ln) {
  // cout << "Register: " << ln->t << "\n";
  int i, n= N (ln->t);
  if (is_atomic (ln->t[0])) type_count (ln->t[0]->label)++;
  for (i= 1; i < n; i++)
...
unregister_link (soft_link ln) {
  ...
  if (is_atomic (ln->t[0])) {
    ...
    if (type_count (ln->t[0]->label) == 0) type_count->reset (ln->t[0]->label);
  }
  for (i= 1; i < n; i++)
...
get_locus_rendering (string var) {
  if (var == "locus-on-paper") return get_user_preference (var, "change");
  if (var == LOCUS_COLOR) return get_user_preference (var, "#404080");
  if (var == VISITED_COLOR) return get_user_preference (var, "#702070");
  return "";
}
...
get_mirrors (tree ln, string id) {
  if (!is_compound (ln, "link", 4) || ln[0] != "mirror" ||
      !is_compound (ln[2], "id", 1) || !is_atomic (ln[2][0]) ||
      !is_compound (ln[3], "id", 1) || !is_atomic (ln[3][0]))
    return list<tree> ();
...
link_announce (observer obs, modification mod) {
  // cout << "Link event " << mod << "\n";
  for (list<string> ids= pointer_resolve[obs]; !is_nil (ids); ids= ids->next)
    for (list<tree> lns= get_links (compound ("id", ids->item)); !is_nil (lns);
         lns           = lns->next)
