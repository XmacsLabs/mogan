...
list_observer_rep::get_ip (path& ip) {
  return (!is_nil (o1) && o1->get_ip (ip)) | (!is_nil (o2) && o2->get_ip (ip));
}

bool
list_observer_rep::set_ip (path ip) {
  return (!is_nil (o1) && o1->set_ip (ip)) | (!is_nil (o2) && o2->set_ip (ip));
}
...
attach_observer (tree& ref, observer o) {
  insert_observer (ref->data, o);
}

void
detach_observer (tree& ref, observer o) {
  remove_observer (ref->data, o);
}

void
clean_observers (tree& ref) {
  clean_observers (ref->data);
}
