    ...
    for (i= 0; i < n;) {
      if (t->label[i] == '<') {
        ...
        if (starts (ss, "<left-")) {
          ...
          r << tree (LEFT, string (ss (6, N (ss) - 1)));
        }
          ...
          r << tree (RIGHT, string (ss (7, N (ss) - 1)));
        }
        ...
        r << tree (op, string (t->label (start, i)));
      }
      else s << t->label[i++];
    }
    if (s != "") r << s;
...
static string       apply_expand_value_strings[]= {
...
upgrade_apply_expand_value (tree t) {
  int             i;
  hashset<string> H;
  for (i= 0; !is_empty (apply_expand_value_strings[i]); i++)
...
static string var_rename[]= {"shrinking factor",
                             "sfactor",
                             "info flag",
                             "info-flag",
...
cached_renamer (string* T, hashmap<string, string>& H) {
  if (N (H) == 0) {
    int i;
    for (i= 0; !is_empty (T[i]); i+= 2)
      H (T[i])= T[i + 1];
  }
  return H;
}
