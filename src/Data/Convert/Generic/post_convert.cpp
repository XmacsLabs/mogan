...
seems_buggy_html_paste (string s) {
  return occurs ("Version:", s) && occurs ("StartHTML:", s) &&
         occurs ("EndHTML:", s) && occurs ("StartFragment:", s) &&
...
default_with_simplify (tree t) {
  ...
  else if (is_func (t, DOCUMENT) || is_func (t, CONCAT) ||
           is_compound (t, "itemize") || is_compound (t, "enumerate") ||
