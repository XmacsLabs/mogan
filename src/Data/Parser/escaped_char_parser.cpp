...
escaped_char_parser_rep::can_parse_hex_with_8_bits (string s, int pos) {
  ...
  return hex_with_8_bits () && s[pos] == 'x' && remaining >= 3 &&
         is_hex_digit (s[pos + 1]) && is_hex_digit (s[pos + 2]);
}

bool
escaped_char_parser_rep::can_parse_hex_with_16_bits (string s, int pos) {
  int remaining= N (s) - pos;
  return hex_with_16_bits () && s[pos] == 'u' && remaining >= 5 &&
         is_hex_digit (s[pos + 1]) && is_hex_digit (s[pos + 2]) &&
         is_hex_digit (s[pos + 3]) && is_hex_digit (s[pos + 4]);
}

bool
escaped_char_parser_rep::can_parse_hex_with_32_bits (string s, int pos) {
  int remaining= N (s) - pos;
  return hex_with_32_bits () && s[pos] == 'U' && remaining >= 9 &&
         is_hex_digit (s[pos + 1]) && is_hex_digit (s[pos + 2]) &&
         is_hex_digit (s[pos + 3]) && is_hex_digit (s[pos + 4]) &&
         is_hex_digit (s[pos + 5]) && is_hex_digit (s[pos + 6]) &&
         is_hex_digit (s[pos + 7]) && is_hex_digit (s[pos + 8]);
}

bool
escaped_char_parser_rep::can_parse_octal_upto_3_digits (string s, int pos) {
  int remaining= N (s) - pos;
  return octal_upto_3_digits () && remaining >= 1 &&
...
escaped_char_parser_rep::do_parse (string s, int& pos) {
  ...
  if (contains (s[pos + 1], m_chars)) pos= pos + 2;
}
