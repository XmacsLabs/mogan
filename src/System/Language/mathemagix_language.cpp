...
void
mathemagix_language_rep::hyphenate (string s, int after, string& left,
                                    string& right) {
  left = s (0, after);
  right= s (after, N (s));
}
...
mathemagix_language_rep::belongs_to_identifier (char c) {
  return (is_digit (c) || is_alpha (c) || c == '_' || c == '$' || c == '%' ||
          c == '?');
}
...
void
mathemagix_language_rep::parse_keyword (hashmap<string, string>& t, string s,
                                        int& pos) {
  int i= pos;
  if (pos >= N (s)) return;
  if (is_digit (s[i])) return;
  while ((i < N (s)) && belongs_to_identifier (s[i]))
    i++;
  string r= s (pos, i);
  if (t->contains (r) && t (r) == "keyword-color") {
    pos= i;
    return;
  }
}
...
void
mathemagix_language_rep::parse_modifier (hashmap<string, string>& t, string s,
                                         int& pos) {
  int i= pos;
  if (pos >= N (s)) return;
  if (is_digit (s[i])) return;
  while ((i < N (s)) && belongs_to_identifier (s[i]))
    i++;
  string r= s (pos, i);
  if (t->contains (r) && t (r) == "modifier-color") {
    pos= i;
    return;
  }
}
...
void
mathemagix_language_rep::parse_class (hashmap<string, string>& t, string s,
                                      int& pos) {
  int i= pos;
  if (pos >= N (s)) return;
  if (is_digit (s[i])) return;
...
void
mathemagix_language_rep::parse_postfix (hashmap<string, string>& t, string s,
                                        int& pos) {
  int i= pos;
  if (pos >= N (s)) return;
  if (is_digit (s[i])) return;
...
void
mathemagix_language_rep::parse_constant (hashmap<string, string>& t, string s,
                                         int& pos) {
  int i= pos;
  if (pos >= N (s)) return;
  if (is_digit (s[i])) return;
  while ((i < N (s)) && belongs_to_identifier (s[i]))
    i++;
  string r= s (pos, i);
  if (t->contains (r) && t (r) == "constant-color") {
    pos= i;
    return;
  }
}
...
parse_number (string s, int& pos) {
  int i= pos;
  if (pos >= N (s)) return;
  if (i + 3 <= N (s) && s[i] == '0' && (s[i + 1] == 'x' || s[i + 1] == 'X') &&
      is_hex_digit (s[i + 2])) {
    i+= 2;
    while (i < N (s) && is_hex_digit (s[i]))
      i++;
    pos= i;
    return;
  }
  ...
  while (i < N (s) &&
         (is_digit (s[i]) ||
          (s[i] == '.' && (i + 1 < N (s)) &&
           (is_digit (s[i + 1]) || s[i + 1] == 'e' || s[i + 1] == 'E'))))
    i++;
  if (i == pos) return;
...
mathemagix_language_rep::get_color (tree t, int start, int end) {
  ...
  do {
    ...
    do {
      ...
      if (opos < pos && possible_type == true) return "type-color";
  ...
  if (type == "identifier" && possible_function == false &&
