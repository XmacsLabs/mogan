...
text_property_rep tp_thin_space_rep (TP_THIN_SPACE, SPC_NONE, SPC_THIN_SPACE, 0,
                                     0);
text_property_rep tp_space_rep (TP_SPACE, SPC_NONE, SPC_SPACE, 0, 0);
text_property_rep tp_space_before_rep (TP_SPACE, SPC_SPACE, SPC_NONE, 0, 0);
text_property_rep tp_dspace_rep (TP_DSPACE, SPC_NONE, SPC_DSPACE, 0, 0);
text_property_rep tp_nb_thin_space_rep (TP_NB_THIN_SPACE, SPC_NONE,
                                        SPC_THIN_SPACE, 0, HYPH_INVALID);
text_property_rep tp_nb_space_rep (TP_NB_SPACE, SPC_NONE, SPC_SPACE, 0,
                                   HYPH_INVALID);
text_property_rep tp_nb_dspace_rep (TP_NB_DSPACE, SPC_NONE, SPC_DSPACE, 0,
                                    HYPH_INVALID);
text_property_rep tp_period_rep (TP_PERIOD, SPC_NONE, SPC_PERIOD, 0, 0);
text_property_rep tp_cjk_normal_rep (TP_CJK_NORMAL, SPC_NONE, SPC_CJK_NORMAL, 0,
                                     0);
text_property_rep tp_cjk_no_break_rep (TP_CJK_NO_BREAK, SPC_NONE,
                                       SPC_CJK_NORMAL, 0, HYPH_INVALID);
text_property_rep tp_cjk_period_rep (TP_CJK_PERIOD, SPC_NONE, SPC_CJK_PERIOD,
                                     HYPH_INVALID, 0);
text_property_rep tp_cjk_wide_period_rep (TP_CJK_PERIOD, SPC_NONE,
                                          SPC_CJK_WIDE_PERIOD, HYPH_INVALID, 0);
text_property_rep tp_cjk_no_break_period_rep (TP_CJK_PERIOD, SPC_NONE,
                                              SPC_CJK_PERIOD, HYPH_INVALID,
                                              HYPH_INVALID);
text_property_rep tp_half_rep (TP_OPERATOR, SPC_NONE, SPC_HALF, 0,
                               HYPH_INVALID);
text_property_rep tp_operator_rep (TP_OPERATOR, SPC_NONE, SPC_OPERATOR, 0,
                                   HYPH_INVALID);
text_property_rep tp_short_apply_rep (TP_OPERATOR, SPC_NONE, SPC_SHORT_APPLY, 0,
                                      HYPH_INVALID);
text_property_rep tp_apply_rep (TP_OPERATOR, SPC_NONE, SPC_APPLY, 0,
                                HYPH_INVALID);
...
text_property_rep::text_property_rep (int type2, int spc_before2,
                                      int spc_after2, int pen_before2,
                                      int pen_after2, int op_type2,
                                      int priority2, int limits2,
                                      tree_label macro2)
    : type (type2), spc_before (spc_before2), spc_after (spc_after2),
      pen_before (pen_before2), pen_after (pen_after2), op_type (op_type2),
      priority (priority2), limits (limits2), macro (macro2) {}

bool
operator== (text_property_rep tpr1, text_property_rep tpr2) {
  return (tpr1.type == tpr2.type) && (tpr1.spc_before == tpr2.spc_before) &&
         (tpr1.spc_after == tpr2.spc_after) &&
         (tpr1.pen_before == tpr2.pen_before) &&
         (tpr1.pen_after == tpr2.pen_after) && (tpr1.op_type == tpr2.op_type) &&
         (tpr1.priority == tpr2.priority) && (tpr1.limits == tpr2.limits) &&
...
copy (text_property_rep tpr) {
  return text_property_rep (tpr.type, tpr.spc_before, tpr.spc_after,
                            tpr.pen_before, tpr.pen_after, tpr.op_type,
                            tpr.priority, tpr.limits, tpr.macro);
}
...
static inline void
set_status (int op1, int op2, int st) {
  succession_status_table[op1 * OP_TOTAL + op2]= st;
}

static inline void
init_could_end (int op) {
...
hashmap<string, int> language_rep::color_encoding (type_helper<int>::init, 32);
...
initialize_color_decodings (string lan_name) {
  ...
  lan->color_decoding (5)=
      get_preference (pfx * "preprocessor_directive", "#20a000");
  lan->color_decoding (10)= get_preference (pfx * "constant", "#4040c0");
  lan->color_decoding (11)=
      get_preference (pfx * "constant_identifier", "#4040c0");
  lan->color_decoding (12)=
      get_preference (pfx * "constant_function", "#4040c0");
  lan->color_decoding (13)= get_preference (pfx * "constant_type", "#4040c0");
  lan->color_decoding (14)=
      get_preference (pfx * "constant_category", "#4040c0");
  ...
  lan->color_decoding (17)=
      get_preference (pfx * "constant_string", "dark grey");
  lan->color_decoding (18)= get_preference (pfx * "constant_char", "#333333");
  lan->color_decoding (20)= get_preference (pfx * "variable", "#606060");
  lan->color_decoding (21)=
      get_preference (pfx * "variable_identifier", "#204080");
  lan->color_decoding (22)=
      get_preference (pfx * "variable_function", "#606060");
  lan->color_decoding (23)= get_preference (pfx * "variable_type", "#00c000");
  lan->color_decoding (24)=
      get_preference (pfx * "variable_category", "#00c000");
  ...
  lan->color_decoding (31)=
      get_preference (pfx * "declare_identifier", "#0000c0");
  lan->color_decoding (32)=
      get_preference (pfx * "declare_function", "#0000c0");
...
void
hyphenless_language_rep::hyphenate (string s, int after, string& left,
                                    string& right) {
...
void
ad_hoc_language_rep::hyphenate (string s, int after, string& left,
                                string& right) {
  if (hyphens->contains (s)) {
...
ad_hoc_language (language base, tree hyphs) {
  ...
  if (!abbrevs->contains (hyphs)) abbrevs (hyphs)= N (abbrevs);
  ...
  if (spell_busy->contains (lan)) {
    ...
    if (f == s) {
      ...
      if (s == uni_upcase_all (s) && is_tuple (r))
        for (int i= 1; i < N (r); i++)
          if (is_atomic (r[i])) r[i]= uni_upcase_all (r[i]->label);
      return r;
    }
  }
  else {
