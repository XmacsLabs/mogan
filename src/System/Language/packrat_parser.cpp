...
packrat_parser_rep::packrat_parser_rep (packrat_grammar gr)
    : lan_name (gr->lan_name), grammar (gr->grammar),
      productions (gr->productions), properties (gr->properties),
      current_tree (packrat_uninit), current_string (""), current_start (-1),
      current_end (-1), current_path_pos (-1), current_pos_path (-1),
      current_cursor (-1), current_input (), current_cache (PACKRAT_UNDEFINED),
...
packrat_parser_rep::set_input (tree t) {
  ...
  current_tree  = t;
  serialize (t, path ());
  if (DEBUG_FLATTEN) debug_packrat << "Input " << current_string << "\n";
...
packrat_parser_rep::encode_path (tree t, path p, path pos) {
  ...
  if (is_nil (pos) || !current_start->contains (p)) return -1;
    ...
    if (current_path_pos->contains (p * pos)) return current_path_pos[p * pos];
...
packrat_parser_rep::decode_path (tree t, path p, int pos) {
  ...
  // cout << "Range " << current_start[p] << " -- " << current_end[p] << "\n";
  if (is_atomic (t)) {
    if (current_pos_path->contains (pos)) return current_pos_path[pos];
...
packrat_parser_rep::parse (C sym, C pos) {
  ...
  if (DEBUG_PACKRAT)
    debug_packrat << "Parse " << packrat_decode[sym] << " at " << pos << INDENT
                  << LF;
  if (sym >= PACKRAT_TM_OPEN) {
    ...
    case PACKRAT_REPEAT:
      ...
      break;
    case PACKRAT_RANGE:
      if (pos < N (current_input) && current_input[pos] >= inst[1] &&
    ...
    case PACKRAT_EXCEPT:
      im= parse (inst[1], pos);
      if (im != PACKRAT_FAILED)
        if (parse (inst[2], pos) != PACKRAT_FAILED) im= PACKRAT_FAILED;
      break;
    ...
    case PACKRAT_TM_ANY:
      ...
      while (true) {
        ...
        if (im == PACKRAT_FAILED) im= parse (PACKRAT_TM_LEAF, old);
        else {
          im= parse (PACKRAT_TM_ARGS, im);
          if (im != PACKRAT_FAILED) im= parse (encode_token ("</>"), im);
        }
        if (old == im) break;
      }
      break;
  ...
  if (DEBUG_PACKRAT)
    debug_packrat << UNINDENT << "Parsed " << packrat_decode[sym] << " at "
                  << pos << " -> " << im << LF;
  return im;
}
...
void
packrat_parser_rep::context (C sym, C pos, C w1, C w2, int mode, array<C>& kind,
  ...
  if (true) {
    ...
    if (properties->contains (sel_key) && properties[sel_key] == "inside")
      ;
    else if (properties->contains (foc_key) &&
             properties[foc_key] == "disallow" && mode == 2)
      ;
    else {
      int n= N (kind);
  ...
  if (is_left_recursive (sym) && mode == 0) {
    ...
    if (i1 == i2) {
      ...
      for (i= n; i < N (kind); i++)
        if (is_selectable (kind[i])) return;
      kind->resize (n);
      begin->resize (n);
      end->resize (n);
    }
...
void
packrat_parser_rep::compress (array<C>& kind, array<C>& begin, array<C>& end) {
  array<C> new_kind, new_begin, new_end;
  for (int i= 0; i < N (kind); i++) {
    int n= N (new_kind);
    if (is_selectable (kind[i]))
      if (N (new_kind) == 0 || new_kind[n - 1] != kind[i] ||
...
consistent_enlargement (tree t, int& begin, int& end) {
  while (begin > 0 || end < N (t)) {
    while (begin > 0 && !empty_line (t[begin - 1]))
      begin--;
    while (end < N (t) && !empty_line (t[end]))
      end++;
...
bool
packrat_select (string lan, string s, tree in, path in_pos, path& p1, path& p2,
                int mode) {
...
packrat_highlight (string lan, string s, tree in) {
  ...
  if (is_func (in, DOCUMENT)) {
    ...
    for (begin= 0; begin < N (in); begin++)
      if (!has_highlight (in[begin], hl_lan)) break;
    for (end= N (in) - 1; end > begin; end--)
      if (!has_highlight (in[end - 1], hl_lan)) break;
