...
void
scilab_language_rep::hyphenate (string s, int after, string& left,
                                string& right) {
  left = s (0, after);
  right= s (after, N (s));
}
...
string
scilab_language_rep::parse_keywords (hashmap<string, string>& t, string s,
                                     int& pos) {
  int i= pos;
  if (pos >= N (s)) return "";
...
string
scilab_language_rep::parse_operators (hashmap<string, string>& t, string s,
                                      int& pos) {
  int i;
  for (i= 11; i >= 1; i--) {
