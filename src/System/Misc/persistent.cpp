...
void
persistent_write (url dir, string key, string val, url file,
                  unsigned int code) {
  string v= local_prefix (dir) * key;
  if (is_directory (file)) {
    ...
    hashmap<string, string> map ("");
    if (is_regular (file)) map= persistent_read_map (file);
    map (key)= val;
...
persistent_make_file_name (url dir, int nr, string suffix) {
  if (nr < MAX_FILES) return dir * url (string ((char) (97 + nr)) * suffix);
  else {
