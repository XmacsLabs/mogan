...
void TestFontSize::test_is_half_multiple() {
...
void TestFontSize::test_round_to_half_multiple() {
...
void TestFontSize::test_font_size_as_int() {
...
void TestFontSize::test_effective_size() {
  // Set up a font rule
  ...
  font fn_corrected = smart_font("sys-chinese", "rm", "medium", "right", 10.3, 600);
...
void TestFontSize::test_set_get_font_size() {
  // Set up a font rule
  tree which = tree(TUPLE, "roman", "rm", "medium", "right", "$s", "$d");
  tree by = tree(TUPLE, "ec", "ecrm", "$s", "$d");
  font_rule(which, by);
  ...
  // Test set_font_size() with another non-half-multiple (should correct to 12.0)
...
// Test script() function
void TestFontSize::test_script_function() {
  ...
  QVERIFY(qAbs(script(10.3, 0) - 10.5) < 0.001); // 10.3 should correct to 10.5
  QVERIFY(qAbs(script(10.7, 0) - 10.5) < 0.001); // 10.7 should correct to 10.5
...
void TestFontSize::test_smart_font_float_size() {
...
void TestFontSize::test_backward_compatibility() {
  // Set up a font rule
  tree which = tree(TUPLE, "roman", "rm", "medium", "right", "$s", "$d");
  tree by = tree(TUPLE, "ec", "ecrm", "$s", "$d");
  font_rule(which, by);
  ...
  font fn_float_int = smart_font("sys-chinese", "rm", "medium", "right", 10.0, 600);
...
void TestFontSize::test_virtual_font_float_size() {
  // Set up a font rule
  tree which = tree(TUPLE, "roman", "rm", "medium", "right", "$s", "$d");
  tree by = tree(TUPLE, "ec", "ecrm", "$s", "$d");
  font_rule(which, by);
  ...
  // Test creating virtual_font with non-half-multiple size (should be corrected)
...
void TestFontSize::test_cache_key_generation() {
  // Set up a font rule
  tree which = tree(TUPLE, "roman", "rm", "medium", "right", "$s", "$d");
  tree by = tree(TUPLE, "ec", "ecrm", "$s", "$d");
  font_rule(which, by);
...
void TestFontSize::test_to_tex_font_size() {
  // Integer sizes should stay the same
...
void TestFontSize::test_get_script_size() {
  QSKIP("get_script_size requires edit_env setup which is complex for unit test");
...
void TestFontSize::test_determine_sizes() {
  QSKIP("determine_sizes requires edit_env setup which is complex for unit test");
  // This function requires a full edit_env environment setup
  // which is beyond the scope of a simple unit test.
  // The function signature verification is covered by compilation tests.
}
